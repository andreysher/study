 ...
 *
 * $Id: create_dirs.c 1 2009-09-11 12:26:19Z william $
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/io/ls/C/0/0/io_ls/create_dirs.c $
...
void create_dirs(const dir_tree_descr_t tree) {
    ...
    fprintf(fp, "Directories successfuly written.\nDepth is %i.\n", tree->depth);
    fclose (fp);
...
void create_files_regularly(const dir_tree_descr_t tree, int filesPerDir) {
    ...
    chdir (tree->root); fp= fopen("reg_files_state","w");
    fprintf(fp, "Files successfuly written.\nRegularly\nDepth is %i.\n%i files per dir.\n", tree->depth, filesPerDir);
    fclose (fp);
    /* to not confuse any other function reset working directory */
    chdir(cwd);
}
