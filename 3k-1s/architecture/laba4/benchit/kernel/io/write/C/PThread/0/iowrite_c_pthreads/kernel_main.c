 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/io/write/C/PThread/0/iowrite_c_pthreads/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->codesequence = bi_strdup("mkdir binary-directory-tree; write leaves (=files)");
   infostruct->kerneldescription = bi_strdup("PThread-kernel writing files into a flat directory");
   infostruct->xaxistext = bi_strdup("Data written in bytes");
   infostruct->num_measurements = (int)(penv->DISKSPACE / penv->FILESIZE);
   infostruct->num_processes = penv->NUMCHANNELS;
   infostruct->num_threads_per_process = penv->CHANNELFACTOR;
   ...
   /* setting up y axis texts and properties */
      ...
      infostruct->legendtexts[0] = bi_strdup("average I/O performance");
 
      infostruct->yaxistexts[1] = bi_strdup("byte / s");
      infostruct->selected_result[1] = SELECT_RESULT_HIGHEST;
      infostruct->base_yaxis[1] = 10; //logarythmic axis 10^x
      infostruct->legendtexts[1] = bi_strdup("maximum I/O performance");
