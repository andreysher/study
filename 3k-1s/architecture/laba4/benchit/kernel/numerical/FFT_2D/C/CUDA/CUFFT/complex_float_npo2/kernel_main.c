 ...
 * $URL: svn+ssh://benchit@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/FFT_2D/C/CUDA/CUFFT/complex_float_npo2/kernel_main.c $
...
int setup(mydata_t *params, DT_s valueR, DT_s valueI)
{
    ...
    size_t mem_size = sizeof (DT) * params->maxElements * params->maxElements;
    ...
    for (i = 0; i < params->maxElements * params->maxElements; i++) {
...
int run(mydata_t *params)
{
    cufftHandle plan;
#ifdef PREC_SINGLE
    CUFFT_CHECK(cufftPlan2d(&plan, params->elements, params->elements, CUFFT_C2C));
#else
    CUFFT_CHECK(cufftPlan2d(&plan, params->elements, params->elements, CUFFT_Z2Z));
#endif
    ...
    CUFFT_CHECK(cudaMemcpy(params->hostData[1], params->devData[1], params->elements * params->elements * sizeof (DT),
...
void bi_getinfo(bi_info* infostruct)
{
    ...
    /* get environment variables for the kernel */
    infostruct->codesequence = bi_strdup("cufft2dx");
    infostruct->kerneldescription = bi_strdup("2D Fast Fourier Transform, Non-Powers of 2, single precision, complex data, CUFFT (C)");
...
int bi_entry(void * mdpv, int iproblemSize, double * dresults)
{
    ...
    myinttype ii, jj, imyproblemSize, imyproblemSize2d, numberOfRuns;
    /* cast void* pointer */
    ...
    imyproblemSize2d = imyproblemSize * imyproblemSize;
    ...
    for (jj = 0; jj < imyproblemSize; jj++)
        for (ii = 0; ii < imyproblemSize; ii++) {
            size_t index = jj * imyproblemSize + ii;
    ...
    flops = (double) (5 * imyproblemSize2d * (log2(imyproblemSize2d)) / dtime);

    /* store the FLOPS in results[2] */
    if (invalid == 1) {
        dresults[1] = INVALID_MEASUREMENT;
        printf("Invalid measurement (problemSize = %d x %d)\n",
                imyproblemSize, imyproblemSize);
    } else {
        dresults[1] = flops;
    }
