 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/gauss/F95/0/0/double/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct) {
   ...
   infostruct->codesequence
      = bi_strdup("do i=1,n-1,1#"
                  "  t=1.0D0/A(i,i)#"
                  "  do j=i+1,n#"
                  "    b(j)=b(j)-b(i)*(A(j,i)*t)#"
                  "    do k=i+1,n#"
                  "      A(j,k)=A(j,k)-(A(j,i)*t)*A(i,k)#"
                  "    end do#"
                  "  end do#"
                  "end do#"
                  "x(n)=b(n)/A(n,n)#"
...
void* bi_init(int problemSizemax) {
   ...
   pmydata->A = (FPT*)malloc(m*m * sizeof(FPT));
