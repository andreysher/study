 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/gauss/F95/0/0/double/init_mat.c $
...
int init_mat_c(FPT *A, int sa1, int sa2, FPT *b, int sb, FPT *x, int sx, int lbd1, int ubd1, int lbd2, int ubd2){
   ...
   for (i=lbd1; i<=ubd1; i++) {
      for (j=lbd2; j<=ubd2; j++) {
         A[j*sa1+i] = (i!=j)?((i!=j-4)?((i!=j+4)?0.0E0:18.0E0):18.0E0):3.0E0;
      }
   }
