 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/elementary/F77/0/0/sincos/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct) {
   ...
   infostruct->codesequence
      = bi_strdup("for(a=0; a<LOOPS; a++) do the mathematical operation; ");
   infostruct->kerneldescription
      = bi_strdup("Execution time of mathematical operations SINUS / COSINUS addict to input value");
   infostruct->xaxistext = bi_strdup("Angle (degree)");
   ...
   infostruct->legendtexts[0] = bi_strdup("sinus");

   /* setting up y axis texts and properties */
   infostruct->yaxistexts[1] = bi_strdup("Op/s");
   infostruct->selected_result[1] = SELECT_RESULT_HIGHEST;
   infostruct->base_yaxis[1] = 0; //logarythmic axis 10^x
   infostruct->legendtexts[1] = bi_strdup("cosinus");
...
int bi_entry(void * mdpv, int iproblemSize, double * dresults) {
   ...
   for(v=0; v<2; v++) {
       ...
       */
      dresults[v+1] = (dtime!=INVALID_MEASUREMENT) ? count / dtime
         : INVALID_MEASUREMENT;
   }
