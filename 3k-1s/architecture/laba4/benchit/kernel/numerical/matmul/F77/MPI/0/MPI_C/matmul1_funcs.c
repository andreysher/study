 ...
 *
 * $Id: matmul1_funcs.c 1 2009-09-11 12:26:19Z william $
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/matmul/F77/MPI/0/MPI_C/matmul1_funcs.c $
...
void allocateANDtouch(fds *pmem, int *pisize)
   ...
   pmem->pda = (double*) malloc((*pisize + 1) * (*pisize + 1) * sizeof(double));
   pmem->pdb = (double*) malloc((*pisize + 1) * (*pisize + 1) * sizeof(double));
   pmem->pdc = (double*) malloc((*pisize + 1) * (*pisize + 1) * sizeof(double));
