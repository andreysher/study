 ...
 * $URL: svn+ssh://benchit@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/FFT_3D/C/CUDA/CUFFT/complex_float_po2/kernel_main.c $
...
int setup(mydata_t *params, DT_s valueR, DT_s valueI)
{
    ...
    CHECK_NULL(params);
		...
		size_t maxElements3d = params->maxElements * params->maxElements * params->maxElements;
    size_t mem_size = sizeof (DT) * maxElements3d;
...
int run(mydata_t *params)
{
    cufftHandle plan;
#ifdef PREC_SINGLE
    CUFFT_CHECK(cufftPlan3d(&plan, params->elements, params->elements, 
    	params->elements, CUFFT_C2C));
#else
    CUFFT_CHECK(cufftPlan3d(&plan, params->elements, params->elements, 
    	params->elements, CUFFT_Z2Z));
#endif

#ifdef PREC_SINGLE
    CUFFT_CHECK(cufftExecC2C(plan, (cufftComplex *) params->devData[0], 
    	(cufftComplex *) params->devData[1], CUFFT_FORWARD));
#else
    CUFFT_CHECK(cufftExecZ2Z(plan, (cufftDoubleComplex *) params->devData[0], 
    	(cufftDoubleComplex *) params->devData[1], CUFFT_FORWARD));
#endif

    CUFFT_CHECK(cudaMemcpy(params->hostData[1], params->devData[1], 
    	params->elements * params->elements * params->elements * sizeof (DT),
...
void evaluate_environment(mydata_t* pmydata, int problemSizemax)
{
    myinttype i, m, tmp;
...
void bi_getinfo(bi_info* infostruct)
{
    ...
    /* get environment variables for the kernel */
    infostruct->codesequence = bi_strdup("cufft3dx");
    infostruct->kerneldescription = bi_strdup("3D Fast Fourier Transform, Powers of 2, single precision, complex data, CUFFT (C)");
...
void* bi_init(int problemSizemax)
{
    ...
    evaluate_environment(pmydata, problemSizemax);
...
int bi_entry(void * mdpv, int iproblemSize, double * dresults)
{
    ...
    myinttype ii, jj, kk, imyproblemSize, imyproblemSize3d, numberOfRuns;
    /* cast void* pointer */
    mydata_t* pmydata = (mydata_t*) mdpv;
    int invalid = 0;

    /* calculate real problemSize */
    imyproblemSize = (myinttype) pow(2, (myinttype) bi_get_list_element(iproblemSize));
    pmydata->elements = imyproblemSize;
    
    imyproblemSize3d = imyproblemSize * imyproblemSize * imyproblemSize;
    ...
    /* check for overflows */
    for (kk = 0; kk < imyproblemSize; kk++)
        for (jj = 0; jj < imyproblemSize; jj++)
            for (ii = 0; ii < imyproblemSize; ii++) {
                size_t index = kk * imyproblemSize * imyproblemSize + jj * imyproblemSize + ii;
                if (isnan(pmydata->hostData[1][index].x) || isnan(pmydata->hostData[1][index].y)) invalid = 1;
                if (isinf(pmydata->hostData[1][index].x) || isinf(pmydata->hostData[1][index].y)) invalid = 1;
            }

    /* calculate the used FLOPS */
    flops = (double) (5 * imyproblemSize3d * (log2(imyproblemSize3d)) / dtime);

    /* store the FLOPS in results[2] */
    if (invalid == 1) {
    	dresults[1] = INVALID_MEASUREMENT;
    	printf("Invalid measurement (problemSize = %d x %d x %d)\n", 
    		imyproblemSize, imyproblemSize, imyproblemSize);
    }
    else {
    	dresults[1] = flops;
    }
