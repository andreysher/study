 ...
 * $URL: svn+ssh://benchit@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/FFT_3D/C/CUDA/CUFFT/complex_float_npo2/kernel_main.c $
...
int setup(mydata_t *params, DT_s valueR, DT_s valueI)
{
    ...
    size_t problemSize = params->maxElements * params->maxElements *
            params->maxElements;

    size_t mem_size = sizeof (DT) * problemSize;
...
int run(mydata_t *params)
{
    cufftHandle plan;
#ifdef PREC_SINGLE
    CUFFT_CHECK(cufftPlan3d(&plan, params->elements, params->elements, params->elements, CUFFT_C2C));
#else
    CUFFT_CHECK(cufftPlan3d(&plan, params->elements, params->elements, params->elements, CUFFT_Z2Z));
#endif
...
void evaluate_environment(mydata_t* pmydata, int problemSizemax)
{
    ...
    pmydata->maxElements = (myinttype) m;
}
...
void bi_getinfo(bi_info* infostruct)
{
    ...
    infostruct->kerneldescription = bi_strdup("3D Fast Fourier Transform, Non-Powers of 2, single precision, complex data, CUFFT (C)");
...
{
    mydata_t *pmydata = (mydata_t*) malloc(sizeof (mydata_t));
    if (pmydata == 0) {
