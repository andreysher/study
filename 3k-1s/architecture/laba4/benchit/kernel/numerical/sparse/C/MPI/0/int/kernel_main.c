 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/sparse/C/MPI/0/int/kernel_main.c $
  ...
  if(rank != MPI_UNDEFINED) {
    ...
    if(rank==0) {
      if(pmydata->verify) {
        ...
        if(eps1 != 0) printf("\nERROR: Result of MAT*VEC (m=%i, n=%i) is not the same like JDSxVEC, biggest distance=%i !",m,n,eps1);
        if(eps2 != 0) printf("\nERROR: Result of MAT*VEC (m=%i, n=%i) is not the same like CRSxVEC, biggest distance=%i !",m,n,eps2);
        if(eps3 != 0) printf("\nERROR: Result of MAT*VEC (m=%i, n=%i) is not the same like CCSxVEC, biggest distance=%i !",m,n,eps3);
