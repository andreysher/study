 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/sparse/C/0/0/int/kernel_main.c $
  ...
  */
  ...
  if(pmydata->verify) {
  	...
  	eps3 = compareVector(b0, b3, m);
   	if(eps1 != 0) printf("\nERROR: Result of MAT*VEC (m=%li, n=%li) is not the same like JDSxVEC, biggest distance=%d !",m,n,eps1);
   	if(eps2 != 0) printf("\nERROR: Result of MAT*VEC (m=%li, n=%li) is not the same like CRSxVEC, biggest distance=%d !",m,n,eps2);
   	if(eps3 != 0) printf("\nERROR: Result of MAT*VEC (m=%li, n=%li) is not the same like CCSxVEC, biggest distance=%d !",m,n,eps3);
