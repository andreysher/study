 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/sparse/C/0/0/long/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct)
{
   ...
   if (penv->output) {
      ...
      i = 0;
      if (penv->wMatxVec) {
        infostruct->yaxistexts[i] = bi_strdup("IOPS");
        infostruct->selected_result[i] = SELECT_RESULT_HIGHEST;
        infostruct->base_yaxis[i] = 0; //logarythmic axis 10^x
        infostruct->legendtexts[i] = bi_strdup("IOPS (std MATxVEC)");
        i++;
      }
      ...
      infostruct->selected_result[i] = SELECT_RESULT_HIGHEST;
      infostruct->base_yaxis[i] = 0; //logarythmic axis 10^x
      infostruct->legendtexts[i] = bi_strdup("IOPS (JDSxVEC)");
      i++;

      infostruct->yaxistexts[i] = bi_strdup("IOPS");
      infostruct->selected_result[i] = SELECT_RESULT_HIGHEST;
      infostruct->base_yaxis[i] = 0; //logarythmic axis 10^x
      infostruct->legendtexts[i] = bi_strdup("IOPS (CRSxVEC)");
      i++;

      infostruct->yaxistexts[i] = bi_strdup("IOPS");
      infostruct->selected_result[i] = SELECT_RESULT_HIGHEST;
      infostruct->base_yaxis[i] = 0; //logarythmic axis 10^x
      infostruct->legendtexts[i] = bi_strdup("IOPS (CCSxVEC)");
   }
  ...
  */
  ...
  if(pmydata->verify) {
  	...
  	eps3 = compareVector(b0, b3, m);
   	if(eps1 != 0) printf("\nERROR: Result of MAT*VEC (m=%li, n=%li) is not the same like JDSxVEC, biggest distance=%li !",m,n,eps1);
   	if(eps2 != 0) printf("\nERROR: Result of MAT*VEC (m=%li, n=%li) is not the same like CRSxVEC, biggest distance=%li !",m,n,eps2);
   	if(eps3 != 0) printf("\nERROR: Result of MAT*VEC (m=%li, n=%li) is not the same like CCSxVEC, biggest distance=%li !",m,n,eps3);
