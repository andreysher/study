 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/jacobi/C/PThread/0/n_mallocs/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct) {
   ...
   infostruct->kerneldescription =
      bi_strdup
      ("Jacobi algorithm measuring FLOPS (ij, ji) for change of dimension, for a given number of posix threads, mallocs in bi_entry addicted to actual problemSize");
   if (penv->cpuCount > 0)
...
int bi_entry(void *mdpv, int iproblemSize, double *dresults) {
   ...
   pmydata->a = (double *)malloc(pmydata->maxn * pmydata->maxn * sizeof(double));
   if (pmydata->a == NULL){
      printf("No more core\n");
      exit(127);
   }
   pmydata->b = (double *)malloc(pmydata->maxn * pmydata->maxn * sizeof(double));
   if (pmydata->b == NULL){
      printf("No more core\n");
      exit(127);
   }
   pmydata->f = (double *)malloc(pmydata->maxn * pmydata->maxn * sizeof(double));
   if (pmydata->f == NULL){
      printf("No more core\n");
      exit(127);
   }
   pmydata->diffnormArray = (double *)malloc(pmydata->threadCount * sizeof(double));
