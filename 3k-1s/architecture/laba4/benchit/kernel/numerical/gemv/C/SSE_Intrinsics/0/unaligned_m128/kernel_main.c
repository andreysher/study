 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/gemv/C/SSE_Intrinsics/0/unaligned_m128/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct) {
   ...
   infostruct->kerneldescription =
      bi_strdup("Matrix Vector Multiply (C + SSE)");
...
void *bi_init(int problemSizemax) {
   ...
   mdp->x = (float*) malloc((maxsize * maxsize) * sizeof(float));
   IDL(3, printf("Alloc 1 done\n"));
   mdp->y = (float*) malloc((maxsize) * sizeof(float));
   IDL(3, printf("Alloc 2 done\n"));
   mdp->a = (float*) malloc((maxsize) * sizeof(float));
   IDL(3, printf("Alloc 3 done\n"));

   if ((mdp->a == 0) || (mdp->x == 0) || (mdp->y == 0)) {
      printf("malloc (%ld bytes) failed in bi_init()\n",
             (long)((2.0 + maxsize) * maxsize * sizeof(float)));
      bi_cleanup(mdp);
      exit(127);
   }
      ...
      switch (j) {
         case 1:
            do {
               ...
               for (i = 0; i < numberOfRuns; i++) {
                  sseunalignJI_(imyproblemSize, imyproblemSize, 1.1, 0.9, mdp->a,
                              mdp->x, mdp->y);
               }
               stop = bi_gettime();
         ...
         case 0:
            do {
               ...
               start = bi_gettime();
               for (i = 0; i < numberOfRuns; i++)
                  sseunalignIJ_(imyproblemSize, imyproblemSize, 1.1, 0.9, mdp->a,
                              mdp->x, mdp->y);
               stop = bi_gettime();
               stop = stop - start - dTimerOverhead;
               numberOfRuns = numberOfRuns * 8;
