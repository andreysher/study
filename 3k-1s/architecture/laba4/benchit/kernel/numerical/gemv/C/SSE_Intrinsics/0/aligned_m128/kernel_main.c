 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/numerical/gemv/C/SSE_Intrinsics/0/aligned_m128/kernel_main.c $
...
void *bi_init(int problemSizemax) {
   ...
   mdp->x = _mm_malloc((maxsize * maxsize) * sizeof(float), 16);
   IDL(3, printf("Alloc 1 done\n"));
   mdp->y = _mm_malloc((maxsize) * sizeof(float), 16);
   IDL(3, printf("Alloc 2 done\n"));
   mdp->a = _mm_malloc((maxsize) * sizeof(float), 16);
      ...
      switch (j) {
         case 1:
            do {
               ...
               for (i = 0; i < numberOfRuns; i++) {
                  ssealignJI_(imyproblemSize, imyproblemSize, 1.1, 0.9, mdp->a,
                              mdp->x, mdp->y);
               }
               stop = bi_gettime();
         ...
         case 0:
            do {
               ...
               start = bi_gettime();
               for (i = 0; i < numberOfRuns; i++)
                  ssealignIJ_(imyproblemSize, imyproblemSize, 1.1, 0.9, mdp->a,
                              mdp->x, mdp->y);
               stop = bi_gettime();
               stop = stop - start - dTimerOverhead;
               numberOfRuns = numberOfRuns * 8;
