 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/sort/quicksort/C/0/0/simple/kernel_main.c $
...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("Quicksort");
...
//  quicksort_int(intarray, 0, imyproblemSize);
  quicksort_wikipedia_int(intarray, 0, imyproblemSize-1);
  dend = bi_gettime();
//printf("\nIntarray finished:\n");
  dtime = dend - dstart;
  dtime -= dTimerOverhead;
...
//  quicksort_float(floatarray, 0, imyproblemSize);
  quicksort_wikipedia_flt(floatarray, 0, imyproblemSize-1);
  dend = bi_gettime();
//printf("\nFloatarray finished:\n");
  dtime = dend - dstart;
  dtime -= dTimerOverhead;
...
//  quicksort_double(doublearray, 0, imyproblemSize);
  quicksort_wikipedia_dbl(doublearray, 0, imyproblemSize-1);
  dend = bi_gettime();
