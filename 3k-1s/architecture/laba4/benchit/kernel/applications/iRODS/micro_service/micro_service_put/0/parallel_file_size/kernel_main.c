...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (Micro-Service get): Parallel transfer for a specific file size");
   sprintf(file_info,"Filesize in %s",penv->file_unit);
   ...
   sprintf(file_info,"<msiDataObjPutWithOptions>");
   infostruct->legendtexts[0] = bi_strdup(file_info);
   sprintf(file_info,"<msiDataObjPutWithOptions> [Rate]");
