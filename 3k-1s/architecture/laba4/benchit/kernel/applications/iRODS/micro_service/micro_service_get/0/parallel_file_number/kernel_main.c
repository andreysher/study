...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (Micro-Service get): Parallel transfer for a specific amount of files");
   ...
   sprintf(file_info,"<msiDataObjGet> [Filesize: %d %s]",(penv->file_block_size*penv->file_block_number),penv->file_unit);
   infostruct->legendtexts[0] = bi_strdup(file_info);
   sprintf(file_info,"<msiDataObjGet> [Filesize: %d %s][Rate]",(penv->file_block_size*penv->file_block_number),penv->file_unit);
