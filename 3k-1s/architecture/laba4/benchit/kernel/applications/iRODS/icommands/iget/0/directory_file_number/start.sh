...
then
	...
	iput $IRODS_RESOURCE -fr $PATH_TEMP ./ >/dev/null
  if [ $? -ne 0 ]
  then
		BOOL=0
  fi
...
then
  ...
  echo "Start to get files."
	...
	$BENCHIT_TOOL_TIME -f "%e;%U;%S" -o "$BENCHIT_KERNEL_PATH_TEMP/$FILE" -a iget $IRODS_RESOURCE $IRODS_PROT $IRODS_THREADS -fr "$SUBDIR" "$PATH_TEMP" >/dev/null
