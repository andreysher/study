...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (iget): Parallel transfer for a specific file size");
   ...
   sprintf(file_info,"<iget %s %s %s>",penv->resource,penv->num_threads,penv->protocol);
   infostruct->legendtexts[0] = bi_strdup(file_info);
   sprintf(file_info,"<iget %s %s %s> [Rate]",penv->resource,penv->num_threads,penv->protocol);
