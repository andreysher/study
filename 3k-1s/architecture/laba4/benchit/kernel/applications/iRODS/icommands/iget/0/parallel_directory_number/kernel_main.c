...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (iget): Transfer of directories with a specific amount of files");
   ...
   sprintf(file_info,"<iget -r %s %s %s> [Filesize: %d %s]",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
   infostruct->legendtexts[0] = bi_strdup(file_info);
   sprintf(file_info,"<iget -r %s %s %s> [Filesize: %d %s][Rate]",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
