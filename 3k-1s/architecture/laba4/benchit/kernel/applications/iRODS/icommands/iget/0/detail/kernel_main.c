...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (iget): Detailed parallel transfer for a specific amount of files");
   ...
   infostruct->legendtexts[3] = bi_strdup("Get File");
   if(penv->result_view == 0)
   	sprintf(file_info,"<iget %s %s %s> (Total Time[Filesize: %d %s])",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
   else
    sprintf(file_info,"<iget %s %s %s> (Rate [Filesize: %d %s]) [second scale]",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
