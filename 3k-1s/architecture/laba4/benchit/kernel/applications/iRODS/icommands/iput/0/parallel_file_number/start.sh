...
then
  ...
  echo "Start to copy files."  
	...
	$BENCHIT_TOOL_TIME -f "%e;%U;%S" -o "$BENCHIT_KERNEL_PATH_TEMP/$FILE" -a $PATH_SCRIPT/irods_parallel.sh "$SPECIAL" "$PATH_TEMP" "$BENCHIT_KERNEL_NUMBER_FILES" "$IRODS_RESOURCE" "$IRODS_PROT" "$IRODS_THREADS" "$SUBDIR" >/dev/null
