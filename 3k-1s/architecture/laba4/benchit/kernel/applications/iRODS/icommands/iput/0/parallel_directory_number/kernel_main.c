...
int set_inc(mydata_t * pmydata)
{
   ...
   p = bi_getenv("BENCHIT_KERNEL_DIR_INC_FUNC", 0);
   if (p == NULL) 
      errors++;
   else 
      pmydata->files_func = atoi(p);
   p = bi_getenv("BENCHIT_KERNEL_DIR_MIN", 0);
   if (p == NULL) 
      errors++;
   else 
      pmydata->files_min = atoi(p);
   p = bi_getenv("BENCHIT_KERNEL_DIR_MAX", 0);
   if (p == NULL) 
      errors++;
   else 
   ...
   {   
   	  ...
   	  if(pmydata->files_number < 0)
   	  {
   	  	fprintf(stderr,"\nError: BENCHIT_KERNEL_FILES_NUMBER < 0\n");
   	  	exit(1);
   	  }
   }   
   ...
   {
      p = bi_getenv("BENCHIT_KERNEL_DIR_INC", 0);
      if (p == NULL) 
         errors++;
      else 
   ...
   else
   {
      ...
      {
         fprintf(stderr,"\nError: BENCHIT_KERNEL_DIR_MIN == 0 is not allowed for BENCHIT_KERNEL_DIR_INC_FUNC>0\n");
         exit(1);
      }
      if(pmydata->files_max == 0)
      {
         fprintf(stderr,"\nError: BENCHIT_KERNEL_DIR_MAX == 0 is not allowed for BENCHIT_KERNEL_DIR_INC_FUNC>0\n");
         exit(1);
      }
      ...
      {
      	fprintf(stderr,"\nError: BENCHIT_KERNEL_DIR_INC_FUNC -> Undefined value\n");
      	exit(1);
      }
...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->kerneldescription = bi_strdup("iRods (iput): Transfer of directories with a specific amount of files");
   sprintf(file_info,"Number of Directories [Number of Files: %d]",penv->files_number);
   infostruct->xaxistext = bi_strdup(file_info);
   ...
   sprintf(file_info,"<iput -r %s %s %s> [Filesize: %d %s]",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
   infostruct->legendtexts[0] = bi_strdup(file_info);
   sprintf(file_info,"<iput -r %s %s %s> [Filesize: %d %s][Rate]",penv->resource,penv->num_threads,penv->protocol,(penv->file_block_size*penv->file_block_number),penv->file_unit);
...
void* bi_init(int problemSizemax)
{
   ...
   {
      fprintf(stderr,"Error: BENCHIT_KERNEL_DIR_MIN is not big enough or bigger than BENCHIT_KERNEL_DIR_MAX");
      exit(1);
   }
   
   /* Calculate the minimum and maximum exponent for the different logarithm. */
   int i,j;
   myinttype dir_number, dir_number_temp, min_exp, max_exp;
   myinttype file_number_dir, full_dir;
   if(pmydata->files_func == 1)
   {
   ...
   for(i = 0; i < pmydata->steps; i++)
   {
      ...
      if(pmydata->files_func == 0)
      {
         ...
         {	
            if((i == 0) || (pmydata->files_inc==1))	
               dir_number = pmydata->files_min + 1 + (i * pmydata->files_inc);
            else
               dir_number = pmydata->files_min + (i * pmydata->files_inc);
         }
         else
            dir_number = pmydata->files_min + (i * pmydata->files_inc);
      }
      ...
      {
         if(i == 0)
            dir_number = pmydata->files_min;
         else
            dir_number = pow(2,min_exp + i);
      }
      if(pmydata->files_func == 2)
      {
         if(i == 0)
            ...
            dir_number = pow(10,min_exp + i);
      }
      if(pmydata->files_max < dir_number)
      	dir_number = pmydata->files_max;
      ...
      {
      	file_number_dir = pmydata->files_number / dir_number;
      	full_dir = pmydata->files_number - (file_number_dir * dir_number);
      }
      if(i == 0)
      	dir_number_temp = 1;
     /* Executes all repetitions of one measurement */ 
	   for(j = 0; j < pmydata->number_runs; j++)
	   {
        	fprintf(stdout,"\n\t\t<<<< Directories: %d , Run: %d >>>>\n", dir_number, (j + 1));
        	fflush(stdout);
        	sprintf(help,"$BENCHIT_SPEZIAL_SCRIPT %d %d %d %d %d",dir_number, dir_number_temp, (j + 1), file_number_dir, full_dir);
        	/*Starts the script defined in $BENCHIT_SPEZIAL_SCRIPT*/
        	if (system(help) != 0)
         	fprintf(stderr,"Error: Couldn't start the script %s.", pmydata->path_script);
	   }
      dir_number_temp = dir_number;  
   }
...
int bi_entry(void * mdpv, int iproblemSize, double * dresults)
{
   ...
   {	
      if(fscanf(pmydata->CSV,"%d\n",&file) != EOF)
      {
      	...
      	int file_size = pmydata->file_block_size * pmydata->file_block_number *  pmydata->files_number;	
