...
then
  ...
  for i in `seq 1 $BENCHIT_KERNEL_NUMBER_META`
  do 
  	if [ $BENCHIT_KERNEL_META_ATTRIBUTE -eq 0 ]
    then
    	echo "add -d $SUBDIR/$SPECIAL benchit benchit_$i" >> "$PATH_TEMP/meta.txt"
		else
		 	echo "add -d $SUBDIR/$SPECIAL benchit_$i benchit" >> "$PATH_TEMP/meta.txt"	 
		fi 
	done
  echo "quit" >> "$PATH_TEMP/meta.txt"
 	#time: real_time;user_time;system_time
	$BENCHIT_TOOL_TIME -f "%e;%U;%S" -o "$BENCHIT_KERNEL_PATH_TEMP/$FILE" -a $PATH_SCRIPT/irods_parallel.sh "$PATH_TEMP/meta.txt"
