 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/communication/communicate/C/MPI/0/blocking/communicate.c $
   ...
   switch (numfct) {
      case 0:
         if (rank==0) {
            ...
            for (i=1; i<size; i++) {
               t1 = bi_gettime();
               MPI_Send(buffer, entries_in_buffer, MPI_FLOAT, i, i*10
                  +1, MPI_COMM_WORLD);
               t2 = bi_gettime();
            ...
            MPI_Recv(buffer, entries_in_buffer, MPI_FLOAT, 0, rank*10
      ...
      case 1:
         if (rank==0) {
            ...
            for (i=1; i<size; i++) {
               t1 = bi_gettime();
               MPI_Ssend(buffer, entries_in_buffer, MPI_FLOAT, i, i
                  *10+2, MPI_COMM_WORLD);
               t2 = bi_gettime();
               tmin = (t2-t1 < tmin) ? t2-t1 : tmin;
               tmax = (t2-t1 > tmax) ? t2-t1 : tmax;
      ...
      case 2:
         if (rank==0) {
            ...
            for (i=1; i<size; i++) {
               t1 = bi_gettime();
               MPI_Rsend(buffer, entries_in_buffer, MPI_FLOAT, i, i
                  *10+3, MPI_COMM_WORLD);
               t2 = bi_gettime();
               tmin = (t2-t1 < tmin) ? t2-t1 : tmin;
               tmax = (t2-t1 > tmax) ? t2-t1 : tmax;
      ...
      case 3:
         ...
         if (rank==0) {
            ...
            for (i=1; i<size; i++) {
               t1 = bi_gettime();
               MPI_Bsend(buffer, entries_in_buffer, MPI_FLOAT, i, i
                  *10+4, MPI_COMM_WORLD);
               t2 = bi_gettime();
               tmin = (t2-t1 < tmin) ? t2-t1 : tmin;
               tmax = (t2-t1 > tmax) ? t2-t1 : tmax;
