 ...
 * $URL: svn+ssh://william@rupert.zih.tu-dresden.de/svn-base/benchit-root/BenchITv6/kernel/communication/communicate/C/MPI/0/blocking/kernel_main.c $
...
/*  Functions:
 *     1. MPI_Send/Recv   ->  rank 0 send to rank 1,...,n
 *     2. MPI_Ssend/Recv  ->  rank 0 send to rank 1,...,n
 *     3. MPI_Rsend/Recv  ->  rank 0 send to rank 1,...,n
 *     4. MPI_Bsend/Recv  ->  rank 0 send to rank 1,...,n
 */
/* attention: in principle the measurement of Bsend should only show the memory bandwidth on rank 0
...
void bi_getinfo(bi_info * infostruct) {
   ...
   infostruct->codesequence
      = bi_strdup("start kernel; make different types auf blocking point-to-point communication;");
   ...
   infostruct->legendtexts[i] = bi_strdup("Send (min time)");
   infostruct->legendtexts[i+1] = bi_strdup("Send (max time)");
   infostruct->legendtexts[i+2] = bi_strdup("Send (mean time)");
   i+=3;
   for (j=i; j<i+3; j++){
   ...
   infostruct->legendtexts[i] = bi_strdup("Send (min bw)");
   infostruct->legendtexts[i+1] = bi_strdup("Send (max bw)");
   infostruct->legendtexts[i+2] = bi_strdup("Send (mean bw)");
   i+=3;
   ...
   for (j=i; j<i+3; j++)
      infostruct->base_yaxis[j] = 10; //logarythmic axis 10^x
   infostruct->legendtexts[i] = bi_strdup("Ssend (min time)");
   infostruct->legendtexts[i+1] = bi_strdup("Ssend (max time)");
   infostruct->legendtexts[i+2] = bi_strdup("Ssend (mean time)");
   i+=3;
   for (j=i; j<i+3; j++)
      infostruct->yaxistexts[j] = bi_strdup("Byte/s");
   for (j=i; j<i+3; j++)
   ...
   for (j=i; j<i+3; j++)
      infostruct->base_yaxis[j] = 0; //logarythmic axis 10^x
   infostruct->legendtexts[i] = bi_strdup("Ssend (min bw)");
   infostruct->legendtexts[i+1] = bi_strdup("Ssend (max bw)");
   infostruct->legendtexts[i+2] = bi_strdup("Ssend (mean bw)");
   i+=3;
   ...
   for (j=i; j<i+3; j++)
      infostruct->base_yaxis[j] = 10; //logarythmic axis 10^x
   infostruct->legendtexts[i] = bi_strdup("Rsend (min time)");
   infostruct->legendtexts[i+1] = bi_strdup("Rsend (max time)");
   infostruct->legendtexts[i+2] = bi_strdup("Rsend (mean time)");
   i+=3;
   for (j=i; j<i+3; j++)
      infostruct->yaxistexts[j] = bi_strdup("Byte/s");
   for (j=i; j<i+3; j++)
   ...
   for (j=i; j<i+3; j++)
      infostruct->base_yaxis[j] = 0; //logarythmic axis 10^x
   infostruct->legendtexts[i] = bi_strdup("Rsend (min bw)");
   infostruct->legendtexts[i+1] = bi_strdup("Rsend (max bw)");
   infostruct->legendtexts[i+2] = bi_strdup("Rsend (mean bw)");
   i+=3;
   ...
   for (j=i; j<i+3; j++)
      infostruct->base_yaxis[j] = 10; //logarythmic axis 10^x
   infostruct->legendtexts[i] = bi_strdup("Bsend (min time)");
   infostruct->legendtexts[i+1] = bi_strdup("Bsend (max time)");
   infostruct->legendtexts[i+2] = bi_strdup("Bsend (mean time)");
   i+=3;
   for (j=i; j<i+3; j++)
      infostruct->yaxistexts[j] = bi_strdup("Byte/s");
   for (j=i; j<i+3; j++)
   ...
   for (j=i; j<i+3; j++)
      infostruct->base_yaxis[j] = 0; //logarythmic axis 10^x
   infostruct->legendtexts[i] = bi_strdup("Bsend (min bw)");
   infostruct->legendtexts[i+1] = bi_strdup("Bsend (max bw)");
   infostruct->legendtexts[i+2] = bi_strdup("Bsend (mean bw)");
   i+=3;
