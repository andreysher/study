...
int functionCount = 1; //Latency+overhead
...
void bi_getinfo(bi_info * infostruct)
{
   ...
   infostruct->xaxistext = bi_strdup("Bank");
...
int bi_entry(void* mdpv, int problemSize, double* results)
{
  ...
  CUDA_CHECK(cudaMemcpy(mdp->d_array,mdp->h_array,arraySize,cudaMemcpyHostToDevice));
	...
	for(i=0; i < valuesPerFunction;i++){
  	dim3 dimBlock(1);
  	dim3 dimGrid(1);
  	CUDA_CHECK_KERNEL_SYNC(testLatency<<<dimGrid, dimBlock, arraySize>>>(mdp->d_array, arrayLength, problemSize, iterations, mdp->d_duration));
...
void evaluate_environment()
{
   ...
   p = bi_getenv("BENCHIT_KERNEL_BANKLIST", 0);
   if(p==0) errors++;
   else bi_parselist(p);
   ...
   if (errors > 0)
   {
      ...
      fprintf(stderr, "BENCHIT_KERNEL_BANKLIST\n");
