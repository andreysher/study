...
int functionCount = 2; //Latency+overhead
...
void bi_getinfo(bi_info * infostruct)
{
   ...
   for(index1 = 0; index1 < valuesPerFunction; index1++){
   	...
   	infostruct->legendtexts[index1+1] = bi_strdup("Latency w loop overhead");
   }
}
...
int bi_entry(void* mdpv, int problemSize, double* results)
{
  ...
  for(i = 0; i < arrayLength; i++)
  	mdp->h_array[i] = (i + problemSize) % arrayLength;
  ...
  CUDA_CHECK(cudaMemcpy(mdp->d_array,mdp->h_array,arraySize,cudaMemcpyHostToDevice));
	...
	for(i=0; i < valuesPerFunction;i++){
  	dim3 dimBlock(1);
  	dim3 dimGrid(1);
  	CUDA_CHECK_KERNEL_SYNC(testLatency<<<dimGrid, dimBlock, arraySize>>>(mdp->d_array, arrayLength, 256, iterations, mdp->d_duration));
  	...
  	results[i+2]=mdp->h_duration[1] / (256 * iterations * 6);
	}
  return 0;
}
...
void evaluate_environment()
{
   ...
   p = bi_getenv("BENCHIT_KERNEL_STRIDELIST", 0);
   if(p==0) errors++;
   else bi_parselist(p);
   ...
   if (errors > 0)
   {
      ...
      fprintf(stderr, "BENCHIT_KERNEL_STRIDELIST\n");
