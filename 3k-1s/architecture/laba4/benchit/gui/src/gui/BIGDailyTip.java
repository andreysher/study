/*********************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGDailyTip.java Author:
 * Robert Wloch Last change by: $Author: tschuet $ $Revision: 1.2 $ $Date: 2007/05/08 09:39:56 $
 *********************************************************************/
package gui;
...
public class BIGDailyTip extends JDialog {
	...
	private final GUIIconPanel iconPanel;
	...
	private final system.BIGConfigFileParser parser;
	...
	public BIGDailyTip(Frame parent, boolean modal) {
		...
		try {
			helpTextBase = bigInterface.getGUIPath() + File.separator
					+ parser.stringCheckOut(configFileTipFileBase);
			helpIconBase = bigInterface.getGUIPath() + File.separator
					+ parser.stringCheckOut(configFileTipIconBase);
		} catch (Exception e) {
			bigInterface.getConsole().postMessage(
					"ERROR no \"" + configFileTipFileBase + "\" or \"" + configFileTipIconBase
							+ "\" entry in BGUI.cfg!", BIGConsole.ERROR);
			helpTextBase = null;
			helpIconBase = null;
		...
		mainPanel.setBorder(BorderFactory.createEmptyBorder(INSET, INSET, INSET, INSET));

		JPanel buttonPanel = new JPanel(new GridBagLayout());
		GridBagConstraints c = new GridBagConstraints();
		// checkbox for deactivating startup daily tips
		JCheckBox checkBox = new JCheckBox("Show tips on startup", showTips);
	...
	/**
	 * Loads the next daily tip. The next index should be stored in the BGUI.cfg file. If not, 0 is used. If the index
	 * stored is greater than the number of available daily tips, the last daily tip will be choosen. If the index points
	 * to the last daily tip before entering this method, it will point to the first one after leaving.
	 * 
	...
	private void loadNextTip(boolean prev) {
		...
		try {
			...
			bigInterface.getConsole().postMessage(
					"ERROR loading the next daily tip: " + helpTexts[nextHelpText - 1], BIGConsole.ERROR);
		}
	...
	/**
	 * Scans a directory for available daily tip files. The directory MUST be set in the BGUI.cfg file. Valid daily tip
	 * files are all html files starting with "tip".
	 */
	...
	/**
	 * Internal class representing the icon panel of the daily tip dialog.
	 */
	 ...
	 */
		...
		/**
		 * If more than one icon exists in the icon base directory a click on the next button will cycle through them.
		 */
		...
		/**
		 * Scan a directory for availably icons. Icons must confirm to the png file format, start with "tip" and should not
		 * be higher than 150 points. The directory must be stored in the BGUI.cfg file.
		 */
