/******************************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGTextField.java Author:
 * SWTP Nagel 1 Last change by: $Author: tschuet $ $Revision: 1.4 $ $Date: 2008/06/02 10:17:17 $
 ******************************************************************************/
package gui;
...
/**
 * This creates a TextField by a specified type. So only type-specified values where accepted, otherwise an error
 * occurs. You can combine this <code>BIGTextField</code> with a <code>BIGSpinner</code> so you can spinn your values
 * (useful for number formats). <br>
 * <br>
 ...
 * @see BIGSpinner
 */
public class BIGTextField extends JTextField implements ActionListener, FocusListener {
	/**
	 * 
	 */
	...
	/** The <code>BIGTextField</code> only accept restricted text-values for this TextField */
	public static final int RESTRICTEDTEXT = 7;
	...
	public BIGTextField(Object value, int type) {
		...
		oldValue = "";
		fieldType = type;
		if (type == RESTRICTEDTEXT) {
			setInputVerifier(new RestrictedTextVerifier());
		}
	...
	public BIGTextField(Object value, int col, int type) {
		this(value, col, type, false);
	}

	/**
	 * Create a TextField with a value, the number of columns and a specified type for this value. Although you can decide
	 * if the <code>BIGTextField</code> must have a value or not.
	 * 
	 * @param value the value of the TextField
	 * @param col the number of columns of the TextField
	 * @param type the type of the <code>BIGTextField</code>
	 ...
	 */
	public BIGTextField(Object value, int col, int type, boolean valueNecessary) {
		// please pay attention: value can be NULL !!!
		super(col);
		oldValue = "";
		fieldType = type;
		if (type == RESTRICTEDTEXT) {
		...
		if (((value == null) || value.equals("")) && valueNecessary) {
	...
	/**
	 * Create a TextField the number of columns. The default type is FLOAT.
	 * 
	 * @param col the number of columns of the TextField
	 */
	...
	/**
	 * Create a TextField. The default type is FLOAT.
	 */
	public BIGTextField() {
		super();
		oldValue = "";
	...
	public void focusLost(FocusEvent e) {
		if (!errorMessage && (fieldType != RESTRICTEDTEXT)) {
			checkFormat(getText());
		}
	}
		...
		if (!text.equals("")) {
			...
			try {
				if (fieldType == INTEGER) {
					...
					if (index != -1) {
						String suffix = (index == 0 ? "0" : text.substring(0, index));
					...
					setText(text);
				} else if (fieldType == RESTRICTEDTEXT) {
					if ((text.indexOf(".") >= 0) || (text.indexOf(" ") >= 0)) {
						printError(TEXT_FORMAT_ERROR);
					}
					setText(text);
	...
	private void printError(int errorType) {
		...
		if (errorType == NUMBER_FORMAT_ERROR) {
			JOptionPane.showMessageDialog(null, "The value has an invalidate format.", "Error",
		...
		} else if (errorType == TEXT_FORMAT_ERROR) {
			JOptionPane.showMessageDialog(null,
					"The value has an invalid format. Please remove all '.'s and blanks.", "Error",
					JOptionPane.ERROR_MESSAGE);
		} else if (getType() == INTEGER) {
			if (errorType == MINIMUM_ERROR) {
				if (getMaximum() != null) {
					JOptionPane.showMessageDialog(null, "The value is not in the range "
							+ getMinimum().intValue() + " till " + getMaximum().intValue() + ".", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null, "The value must be greater than "
							+ getMinimum().intValue(), "Error", JOptionPane.ERROR_MESSAGE);
				}
				...
				if (getMinimum() != null) {
					...
					JOptionPane.showMessageDialog(null, "The value must be smaller than "
			...
			if (errorType == MINIMUM_ERROR) {
				if (getMaximum() != null) {
					JOptionPane.showMessageDialog(null, "The value is not in the range " + getMinimum()
							+ " till " + getMaximum() + ".", "Error", JOptionPane.ERROR_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null, "The value must be greater than " + getMinimum(),
				...
				if (getMinimum() != null) {
					...
					JOptionPane.showMessageDialog(null, "The value must be smaller than " + getMaximum(),
							"Error", JOptionPane.ERROR_MESSAGE);
				}
			}
			...
			if (errorType == MINIMUM_ERROR) {
				if (getMaximum() != null) {
					JOptionPane.showMessageDialog(null, "The value is not in the range "
							+ getMinimum().doubleValue() + " till " + getMaximum().doubleValue() + ".", "Error",
							JOptionPane.ERROR_MESSAGE);
				} else {
					JOptionPane.showMessageDialog(null, "The value must be greater than "
							+ getMinimum().doubleValue(), "Error", JOptionPane.ERROR_MESSAGE);
				}
			} else if (errorType == MAXIMUM_ERROR) {
				if (getMinimum() != null) {
					JOptionPane.showMessageDialog(null, "The value is not in the range "
			...
			if (errorType == DATE_ERROR) {
				JOptionPane.showMessageDialog(null, "The value is not a validate date.", "Error",
						JOptionPane.ERROR_MESSAGE);
			} else if (errorType == MINIMUM_ERROR) {
				JOptionPane.showMessageDialog(null, "The date must be after today", "Error",
						JOptionPane.ERROR_MESSAGE);
			}
		}
		errorMessage = false;
	 ...
	 * 
	 * @param type FLOAT - only float values were accepted, INTEGER - only integer values were accepted, TEXT - all values
	 *          were accepted. ... see top.
	 */
	...
	public void setDate(Calendar date) {
		if (getType() == DATE && date != null) {
			...
			String text = (day < 9 ? "0" : "") + day + "." + (month < 9 ? "0" : "") + month + "." + year;
			setText(text);
		}
	}
	...
	/**
	 * Returns the actually value of this TextField in a percent value as Double. i.e.: 16% --> 0.16
	 */
	...
	public Calendar getDate() {
		...
		if (!getText().equals("")) {
			...
			GregorianCalendar cal = new GregorianCalendar(year, month, date, 0, 0, 0);
			return cal;
		} else
			return null;
	}
	...
	/**
	 * sets the number of decimals after the comma, (if FieldType == CURRENCY --> decimalCount := 2, so the value will be
	 * round to 2 decimals after comma. i.e. the value is 157.999 the rounded value is: 158.<b>00</b>).
	 * 
	...
	/**
	 * Returns the number of decimals after the comma, (i.e. if fieldType == CURRENCY --> getDecimalCount = 2,
	 * 158.<b>00</b>).
	 * 
	...
	/** set to true, when it is necessary to put in a value in this TextField. */
	...
	/**
	 * Connect a <code>JTextField</code> with another <code>JTextField</code>. So all connected <code>JTextField</code>s
	 * where changed, when the value of one changed.
	 * 
	...
	// variables
	private static final int MINIMUM_ERROR = 1, MAXIMUM_ERROR = 2, NUMBER_FORMAT_ERROR = 3,
			VALUE_IS_NESSESSARY_ERROR = 4, DATE_ERROR = 5, TEXT_FORMAT_ERROR = 6;
	...
	private final ArrayList<BIGTextField> connectionList = new ArrayList<BIGTextField>();

	class RestrictedTextVerifier extends InputVerifier {
		@Override
		public boolean verify(JComponent input) {
			String tfText = ((JTextField) input).getText();
			if ((tfText.indexOf(".") >= 0) || (tfText.indexOf(" ") >= 0)) {
				// text contains forbidden characters
				JOptionPane.showMessageDialog(null,
						"The value has an invalid format. Please remove all '.'s and blanks.", "Error2",
						JOptionPane.ERROR_MESSAGE);
				return false;
			} else
				// text content of textfield is valid
				return true;
		}
	}
}
