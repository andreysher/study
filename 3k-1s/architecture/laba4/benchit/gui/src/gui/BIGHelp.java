/******************************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGHelp.java Author: SWTP
 * Nagel 1 Last change by: $Author: tschuet $ $Revision: 1.4 $ $Date: 2007/07/10 10:41:30 $
 ******************************************************************************/
package gui;
...
public class BIGHelp extends JFrame {
	...
	private final BIGInterface db = BIGInterface.getInstance();

	JEditorPane helpPane = new JEditorPane();
	JTree helpTree;
	DefaultListModel<String> searchResultFieldModel = new DefaultListModel<String>();
	JList<String> searchResultField = new JList<String>();
	...
	public BIGHelp() {
		...
		displayHelpText(o);
		setBounds(32, 64, getWidth(), getHeight());
		pack();

	}

	/**
	 * Creates all major GUI components and arranges them. Includes ListSelectionListener for JList, TreeSelectionListener
	 * for JTree and ChangeListener for JTabbedPane.
	 */
	private void initComponents() {
		...
		JScrollPane helpTreeViewScroll = new JScrollPane(helpTree,
		...
		searchResultField.addListSelectionListener(new ListSelectionListener() {
			public void valueChanged(ListSelectionEvent e) {
				// System.err.println("BIGHelp: " + searchResultField.getSelectedIndex() + " : " +
				// e.getValueIsAdjusting());
				if ((found == true) && (searchResultField.getMaxSelectionIndex() >= 0)
						&& (searchResultField.getSelectedIndex() >= 0) && (e.getValueIsAdjusting() == false)) {
		...
		int ypos = 0;
		addComponent(exLabel, ypos++, 0, 1, 2, helpSearchPanel, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.NORTHWEST, 100, 0);
		...
		addComponent(searchStringField, ypos, 0, 1, 1, helpSearchPanel, GridBagConstraints.HORIZONTAL,
				GridBagConstraints.NORTHWEST, 100, 0);
		...
		addComponent(searchButton, ypos++, 1, 1, 1, helpSearchPanel, GridBagConstraints.NONE,
				GridBagConstraints.NORTHEAST, 0, 0);

		searchResultField.setSelectionMode(ListSelectionModel.SINGLE_SELECTION);

		JScrollPane searchResultFieldScroll = new JScrollPane(searchResultField);

		addComponent(searchResultFieldScroll, ypos++, 0, 1, 2, helpSearchPanel,
				GridBagConstraints.BOTH, GridBagConstraints.NORTHWEST, 100, 100);

		JTabbedPane helpTreeView = new JTabbedPane(SwingConstants.TOP, JTabbedPane.WRAP_TAB_LAYOUT);
		...
		helpTreeView.addChangeListener(new ChangeListener() {
			public void stateChanged(ChangeEvent e) {
				...
				if (tabchoice == 0) {
					if (helpTree.getSelectionCount() > 0) {
						setContentForTree();
					} else {
						helpPane.setText("");
					}
		...
		if (node.isLeaf()) {
			...
			if ((((String) node.getUserObject()).startsWith("BIG"))
				...
				if ((((String) tempNode.getUserObject()).startsWith("BIG"))
					...
					// the xml-entries
					String filename = (String) ((DefaultMutableTreeNode) helpTree.getSelectionPath()
							.getParentPath().getLastPathComponent()).getUserObject();
					helpPane.setText(db.getEntry(filename, (String) node.getUserObject()).getHelp());
	...
	private URL createURL(String file) {
		URL baseURL = null;
		String prefix = "file://" + BIGInterface.getInstance().getHelpPath() + File.separator;
	 ...
	 */
	private void displayHelpText(String what) {
		if (what.startsWith("url://")) {
			URL file = createURL(what.substring("url://".length()));
			displayHelpText(file);
		} else {
	...
	private void displayHelpText(URL file) {
		try {
			helpPane.setPage(file);
		} catch (IOException e) {
			System.err.println("BIGHelp: File not found: " + file);
		}
		helpPane.repaint();
	}

	/**
	 * In this method an unlimited amount of HTML files given by their path in filenames are searched for a string located
	 * in searchString. All HTML tags are filtered before. Returns the number of files, in which at least one hit occured.
	 */
	private int searchHTML(String searchString, BIGStrings filenames) {

		try {
			URL u = new URL("http://www.benchit.org/wiki/index.php/Special:Search?search=" + searchString
					+ "&go=Go");
		...
		while (fileIt.hasNext()) {
			...
			if (pureText.indexOf(searchString.toLowerCase()) >= 0) {
				...
				aroundString.append("...");
				searchResultFieldModel.addElement(filename.substring(
						filename.lastIndexOf(File.separator) + 1, filename.length()) + "\t:\t" + aroundString);
				count++;
			}
		}
	...
	/**
	 * Searches the database given by BIGInterface for occurrences of searchString. Returns the number of entry in which a
	 * hit occurred.
	 */
	private int searchEntries(String searchString) {
		...
		while (fileIt.hasNext()) {
			...
			while (entryIt.hasNext()) {
				...
				String text = filename.replaceAll("<hostname>", db.getHost()) + " / " + entry.getViewName()
						+ "<br>-----<br>" + entry.getHelp();
	...
	public void search(String searchString) {
		...
		searchResultFieldModel = new DefaultListModel<String>();

		int count = searchHTML(searchString, htmlFileList) + searchEntries(searchString);
		if (count <= 0) {
			found = false;
	...
	private void createNodes(DefaultMutableTreeNode top) {
		BIGStrings files = getHtmlFileList();
		for (int i = 0; i < files.size(); i++) {
			if (((files.get(i)).indexOf("BIG") > -1) && (files.get(i)).endsWith("html")) {
				top.add(getNodesForFile(files.get(i)));
		...
		BIGStrings tempArray = db.getAllFilenames();
		Iterator<String> tempIterator = tempArray.iterator();
		while (tempIterator.hasNext()) {
			DefaultMutableTreeNode fileNode = new DefaultMutableTreeNode((tempIterator.next()));
			paramsHelp.add(fileNode);

			BIGStrings tempArray2 = db.getAllEntrynames((String) fileNode.getUserObject());
			Iterator<String> tempIterator2 = tempArray2.iterator();
			while (tempIterator2.hasNext()) {
				DefaultMutableTreeNode entryNode = new DefaultMutableTreeNode((tempIterator2.next()));
	...
	private BIGStrings getHtmlFileList() {
		...
		File helpRoot = new File(BIGInterface.getInstance().getHelpPath());
	...
	private DefaultMutableTreeNode getNodesForFile(String file) {
		...
		String fileContent = bs.toString();
		{
			while (fileContent.indexOf("<h2>") > -1) {
				DefaultMutableTreeNode h2Node = new DefaultMutableTreeNode(fileContent.substring(
						fileContent.indexOf("<h2>") + 4, fileContent.indexOf("</h2>")));
				fileContent = fileContent.substring(fileContent.indexOf("</h2>") + 4, fileContent.length());
				node.add(h2Node);
				while ((fileContent.indexOf("<h3>") > -1)
						&& ((fileContent.indexOf("<h3>") < fileContent.indexOf("<h2>")) || (fileContent
								.indexOf("<h2>") == -1))) {
					DefaultMutableTreeNode h3Node = new DefaultMutableTreeNode(fileContent.substring(
							fileContent.indexOf("<h3>") + 4, fileContent.indexOf("</h3>")));
					fileContent = fileContent.substring(fileContent.indexOf("</h3>") + 4,
							fileContent.length());
					h2Node.add(h3Node);
				}
			}
