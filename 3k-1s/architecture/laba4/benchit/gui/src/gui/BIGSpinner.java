/******************************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGSpinner.java Author: SWTP
 * Nagel 1 Last change by: $Author: rschoene $ $Revision: 1.1 $ $Date: 2005/10/20 13:13:57 $
 ******************************************************************************/
package gui;
...
/**
 * This Component creates a spinner for a specified TextField, which increase or decrease the value of the TextField
 * (Integer, Float, Double, ...) by clicking the ArrowUp or ArrowDown button.<br>
 * You can set, how big one strut is.<br>
 * <br>
 * I.e. the value of the TextField is 5.3 the strut is 0.4, so the next value after pressing the up button is 5.7, 6.1,
 * ...
 * 
...
public class BIGSpinner extends JPanel {
	 ...
	 */
	private void createSpinner(BIGTextField field, float strutValue, int type) {
					...
					public void run() {
						...
						while (mousePressed && isEnabled()) {
							if (fieldType == BIGTextField.INTEGER) {
								...
								value += strut;
								if (jtf.getMaximum() != null) {
									float maximum = jtf.getMaximum().floatValue();
									if (value > maximum) {
					...
					public void run() {
						...
						while (mousePressed && isEnabled()) {
							if (fieldType == BIGTextField.INTEGER) {
								...
								value -= strut;
								if (jtf.getMinimum() != null) {
									float minimum = jtf.getMaximum().floatValue();
									if (value < minimum) {
	...
	public void setSize(Dimension d) {
		setPreferredSize(d);
		Dimension buttonDim = new Dimension(incButton.getWidth(), d.height / 2 - 1);
	...
	/**
	 * You can add an <code>ActionListener</code> which occurs for <b>both</b> buttons (inc. and dec. button).
	 * 
