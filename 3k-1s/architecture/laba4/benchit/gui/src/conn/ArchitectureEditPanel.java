...
/**
 * <p>
 * Überschrift: The ArchitectureEditPanel
 * </p>
 * <p>
 * Beschreibung: here you select your Graph with first defining the architecture
 * </p>
 * <p>
...
public class ArchitectureEditPanel extends JScrollPane implements ActionListener {
	/**
	 * 
	 */
	...
	/** a link to this */
	private final ArchitectureEditPanel content;
	 ...
	 */
	public ArchitectureEditPanel(SocketCon socketCon, MainEditPanel root, String[] identifiers) {
		...
		// select which architecture-item
			case 0 : {
				...
				for (int i = 0; i < identifiers.length; i++) {
					...
					jb.setToolTipText("Click this to select from identifier " + identifiers[i]);
			...
			case 1 : {
				...
				for (int i = 0; i < level1CheckBoxes.length; i++) {
					...
					level1CheckBoxes[i].setToolTipText("Select this if you want to select"
							+ level1CheckBoxes[i].getText() + " for " + selectedIdentifier);
				...
				// Button "Back"
				JButton backButton = new JButton("Back", new ImageIcon("img" + java.io.File.separator
						+ "back-16.png"));
				...
				// Button "Go on"
				JButton addButton = new JButton("Go on", new ImageIcon("img" + java.io.File.separator
						+ "goOn-16.png"));
					...
					public void actionPerformed(ActionEvent evt) {
						...
						Thread t = new Thread() {
							...
							public void run() {
								...
								for (int i = 0; i < level1CheckBoxes.length; i++) {
									if (level1CheckBoxes[i].isSelected()) {
										selected[howManySelected] = level1CheckBoxes[i].getText();
										howManySelected++;
									}
								}
								...
								try {
									...
									level2Programnames = socketCon.startArchitectureSelectWork2(selectedIdentifier,
											selected);
									level = 2;
									// build the panel
									t = new Thread() {
				...
				buttonGc.anchor = GridBagConstraints.CENTER;
				JButton backButton2 = new JButton("Back", new ImageIcon("img" + java.io.File.separator
						+ "back-16.png"));
				...
				JButton addButton2 = new JButton("Go on", new ImageIcon("img" + java.io.File.separator
						+ "goOn-16.png"));
			...
			case 2 : {
				...
				for (int i = 0; i < level2Programnames.length; i++) {
					...
					// content means this, so actionPerformed is called
					jb.addActionListener(content);
					mainPanel.add(jb, gc);
				}
				JButton backButton2 = new JButton("Back", new ImageIcon("img" + java.io.File.separator
			...
			case 3 :
				...
				for (int i = 0; i < ((String[]) level3Vector.elementAt(0)).length; i++) {
					gc.gridy = i + 2;
					// looks like a label, bu its a button
					JButton jl = new JButton(((String[]) level3Vector.elementAt(0))[i]);
							...
							for (int jIn = 0; jIn < checkBoxes.length; jIn++) {
								for (int iIn = 0; iIn < checkBoxes[0].length; iIn++)
									if (checkBoxes[jIn][iIn] != null) {
										...
										if ((jIn != row) && (!((evt.getModifiers() & ActionEvent.CTRL_MASK) > 0))) {
				...
				// , so not commented)
				for (int i = 0; i < ((String[]) level3Vector.elementAt(2)).length; i++) {
					gc.gridx = i + 1;
					JButton jl = new JButton(((String[]) level3Vector.elementAt(2))[i]);
					jl.setBorder(BorderFactory.createEmptyBorder());
					final int column = i;
					// see comment above
					jl.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent evt) {
							for (int jIn = 0; jIn < checkBoxes.length; jIn++) {
								for (int iIn = 0; iIn < checkBoxes[0].length; iIn++)
									if (checkBoxes[jIn][iIn] != null) {
										if ((iIn != column) && (!((evt.getModifiers() & ActionEvent.CTRL_MASK) > 0)))
				...
				// setting checkBoxes
				checkBoxes = new JCheckBox[((String[]) level3Vector.elementAt(0)).length][((String[]) level3Vector
						.elementAt(2)).length];

				for (int i = 0; i < ((String[]) level3Vector.elementAt(0)).length; i++) {
					for (int j = 0; j < ((String[]) level3Vector.elementAt(2)).length; j++) {
						...
						String bitmask = Long.toBinaryString((new Long(
								(((String[]) (level3Vector.elementAt(3)))[i]))).longValue());
						// add beginning zeros
						while (bitmask.length() < (((String[]) level3Vector.elementAt(2)).length)) {
							bitmask = '0' + bitmask;
						}
						gc.gridy = i + 2;
						gc.gridx = j + 1;
						if (bitmask.charAt(j) == '1') {
							...
							gridbag.setConstraints(checkBoxes[i][j], gc);
							mainPanel.add(checkBoxes[i][j], gc);
				...
				addButton.setToolTipText("Click here add the Graphs, that are defined by your settings");
				addButton.addActionListener(new ActionListener() {
					public void actionPerformed(ActionEvent evt) {
							...
							public void run() {
								...
								for (int i = 0; i < checkBoxes.length; i++) {
									...
									if (numberOfSelected > 1) {
										...
										settings[0] = ((String[]) level3Vector.get(1))[i];
										// we set the description to <any>! (load it once, as i said)
										settings[1] = "<any>";
										// add
										root.addGraphWithoutReloading(identifiers, settings);
										// ohem... what's THAT???
										// get it slow...
										...
										for (int j = checkBoxes[0].length - 1; j > -1; j--) {
											// if there is a checkbox, so the graph exists
											if (checkBoxes[i][j] != null) {
												// but the checkbox isn't selected
												if (!checkBoxes[i][j].isSelected()) {
													// then we select the graph
													root.root.graphData.setSelectedGraph(root.root.

													graphData.getGraph(base + j));
													// and remove it
													root.root.graphData.removeSelectedGraph();
												}
											}
										}
										// ... so ... got it?
										...
										// and when thats done
										// update the view
										root.root.updateView(root.root.graphData);
										...
										for (int j = 0; j < checkBoxes[0].length; j++) {
											...
											if (checkBoxes[i][j] != null) {
												if (checkBoxes[i][j].isSelected()) {
													...
													identifiers[1] = "Description";
													settings[0] = ((String[]) level3Vector.get(1))[i];
													settings[1] = ((String[]) level3Vector.get(2))[j];
													// aadd it
				...
				// go to level 0
				JButton backToStartButton = new JButton("Back to Begin", new ImageIcon("img"
						+ java.io.File.separator + "backToBegin-16.png"));
				backToStartButton.setToolTipText("Click here to return to the beginning of the selection");
				...
				gc.gridx = 0;
				gc.gridy = ((String[]) level3Vector.elementAt(0)).length + 2;
				...
				addButton2.setToolTipText("Click here add the Graphs, that are defined by your settings");
				addButton2.addActionListener(addButton.getActionListeners()[0]);
				JButton backToStartButton2 = new JButton("Back to Begin", new ImageIcon("img"
						+ java.io.File.separator + "backToBegin-16.png"));
				backToStartButton2.setToolTipText("Click here to return to the beginning of the selection");
				...
				t = new Thread() {
					...
					public void run() {
						...
						try {
							...
							selectedIdentifier = ((JButton) evt.getSource()).getText();
							// what tells us the server
							String[] identifiers2 = socketCon.startArchitectureSelectWork1(selectedIdentifier);
			...
			case 2 :
				...
				(new Thread() {
					...
					public void run() {
						...
						String selectedProgram = ((JButton) evt.getSource()).getText();
						...
						try {
							level3Vector = socketCon.startArchitectureSelectWork3(selectedIdentifier, selected,
									selectedProgram);
						} catch (Exception e) {
							// no exception
						}
