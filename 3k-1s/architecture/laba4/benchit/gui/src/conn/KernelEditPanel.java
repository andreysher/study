 ...
 * </p>
 * <p>
 * Beschreibung: Here you can select your Graph by first defining the kernel
 * </p>
 * <p>
 ...
 * @version 1.0
 */
public class KernelEditPanel extends JScrollPane implements ActionListener {
	...
	/**
	 * contains the actual level. level 0: select algorithm
	 */
	private int level = 0;
	/**
	 ...
	 * reference to this
	 */
	private final KernelEditPanel content;
	/**
	 ...
	 * @param comboBoxEntries the content of the jComboBoxes for the identifiers (elements of this are all String[]s) (not
	 *          needed, you may use null)
	 */

	public KernelEditPanel(SocketCon socketCon, MainEditPanel root, String[] identifiers) {
		this.socketCon = socketCon;
		this.root = root;
		content = this;
		initLayout();
	...
	private void initLayout() {
		...
		// first select kernel
			case 0 :
				final Thread t = new Thread() {
					...
					public void run() {
						...
						// all programnames
						Vector<?> allProgramNames = v.comboContent.get(1);
						...
						for (int i = 0; i < allProgramNames.size(); i++) {
							...
							gc.gridy = i;
							JButton jb = new JButton((String) allProgramNames.elementAt(i));
							jb.setToolTipText("Click here to add a function with kernel:"
			...
			case 1 :
				...
				for (int i = 0; i < ((String[]) level1Vector.elementAt(0)).length; i++) {
					gc.gridy = i + 2;
					// machine
					// computer definitions at the left
					// looks like a label, but its a button
					JButton jl = new JButton(((String[]) level1Vector.elementAt(0))[i]);
					final int row = i;
				...
				gc.gridy = 1;
				// now the columns
				for (int i = 0; i < ((String[]) level1Vector.elementAt(2)).length; i++) {
					gc.gridx = i + 1;
					// description
					JButton jl = new JButton(((String[]) level1Vector.elementAt(2))[i]);
					jl.setBorder(BorderFactory.createEmptyBorder());
					final int column = i;
					jl.addActionListener(new ActionListener() {
						public void actionPerformed(ActionEvent evt) {
				...
				// setting checkBoxes
				checkBoxes = new JCheckBox[((String[]) level1Vector.elementAt(0)).length][((String[]) level1Vector
						.elementAt(2)).length];
				for (int i = 0; i < ((String[]) level1Vector.elementAt(0)).length; i++) {
					for (int j = 0; j < ((String[]) level1Vector.elementAt(2)).length; j++) {
						String bitmask = Long.toBinaryString((new Long(
								(((String[]) (level1Vector.elementAt(3)))[i]))).longValue());
						// got a bitmask
						// 3 could mean 0011 but also 011 or 11 depending on number of descriptions (e.g. ijk or
						// kij)
						// which means ((String[])level1Vector.elementAt( 2 )).length
						// add leading zeros
						while (bitmask.length() < (((String[]) level1Vector.elementAt(2)).length)) {
							bitmask = '0' + bitmask;
						}
						gc.gridy = i + 2;
						gc.gridx = j + 1;
							...
							public void run() {
								...
								for (int i = 0; i < checkBoxes.length; i++) {
									...
									if (numberOfSelected > 1) {
										...
										settings[0] = ((String[]) level1Vector.get(1))[i];
										settings[1] = "<any>";
										...
										for (int j = checkBoxes[0].length - 1; j > -1; j--) {
											if (checkBoxes[i][j] != null) {
												// remove unnecassary
												if (!checkBoxes[i][j].isSelected()) {
													root.root.graphData.setSelectedGraph(root.root.graphData.getGraph(base
															+ j));
										...
										for (int j = 0; j < checkBoxes[0].length; j++) {
											...
											if (checkBoxes[i][j] != null) {
												...
												if (checkBoxes[i][j].isSelected()) {
													...
													identifiers[2] = "Kernelname";
													settings[0] = ((String[]) level1Vector.get(1))[i];
													settings[1] = ((String[]) level1Vector.get(2))[j];
													settings[2] = (kernelName);
