...
/**
 * <p>
 * Überschrift: MainViewPanel
 * </p>
 * <p>
 * Beschreibung: shows the selected Graph(s) by using plot.BIGPlot
 * </p>
 * <p>
...
public class MainViewPanel {
	...
	/**
	 * makes a tabbed pane out of graphs. this is pretty the same as plot.BIGPlot does. but it combines the panels to a
	 * tabbed pane
	 * 
	...
	public static JTabbedPane setData(GraphData graphData) {
		try {
			// get the mixer
			plot.data.BIGResultMixer plotable = graphData.getMixer();
			if (plotable == null) {
				// create empy mixer
				plotable = new BIGResultMixer(null, "BenchIT-DB");
			...
			for (int i = 0; i < graphData.size(); i++) {
				if (plotable.getDataForFunction(graphData.getGraph(i).getGraphName()) != null) {
					plotable.addFunction(graphData.getGraph(i));
				}
			}
			// return the plot
