...
public class BIGFileHelper {
	/**
	 * copies a file (or folder) to a given folder THE NEW FILES WILL NOT HAVE THE EXECUTABLE BIT!!!
	 * 
	...
	public static void copyToFolder(File fileToCopy, File destinationFolder, boolean overwrite) {
		...
		if (fileToCopy.isFile()) {
			File newFile = new File(destinationFolder.getAbsolutePath() + File.separator
					+ fileToCopy.getName());
			if (newFile.exists()) {
				if (!overwrite)
			...
			if (fileInFolder == null)
				// folder is empty -> no files to copy
				return;

			File newFolder = new File(destinationFolder.getAbsolutePath() + File.separator
					+ fileToCopy.getName());
			// make directory
			if (!newFolder.exists() && !newFolder.mkdirs()) {
				System.err.println("Couldn't create directory " + newFolder.getAbsolutePath());
			}
			while (!newFolder.exists()) {
				;
			}

			// now start with copy to folder
	...
	public static String getFileContent(File f) {
		...
		try {
			...
			System.err
					.println("Warning: ASCII-Encoding not supported, String may not work correctly when resaved.");
	...
	public static File[] getAllSubFiles(File f, File[] subFiles, int index) {
		...
		if (f.isDirectory()) {
			...
			for (int i = 0; i < ownSubFiles.length; i++) {
				...
				/*
				 * System.err.println( " subFile " + ownSubFiles[ i ].getName() + " has " + getNumberOfSubFiles( ownSubFiles[ i
				 * ] ) + ", next one will begin at " + index +"+" + getNumberOfSubFiles( ownSubFiles[ i ] ) ) ;
				 */
