...
public class BIGExecuteConsole extends JFrame {
	/**
	 * 
	 */
		...
		getProcessesOutput = new Thread() {
			...
			public void run() {
				while (true) {
					...
					if ((process != null) && (ipRead != null) && (erRead != null)) {
						try {
							if (erRead.ready()) {
								char character;
								while (erRead.ready()) {
							...
							System.err.println(ex);
							System.err.println("Couldn't read output from Process.");
						}
						jtf.repaint();
					}
					try {
			...
			public void keyTyped(KeyEvent ke) {
				...
				if ((typedChar[0] == '\n')) {
					...
					if ((toWriteString.length() < 2)) {
						...
						StringBuffer lastLineOfJTF = new StringBuffer(jtf.getText().substring(0,
								jtf.getText().lastIndexOf("\n")));
						System.err.println(lastLineOfJTF);
						while (lastLineOfJTF.indexOf("\n") != -1) {
							lastLineOfJTF.delete(0, lastLineOfJTF.indexOf("\n") + 1);
						}
						if ((lastLineOfJTF.indexOf("e.g.") != -1)
								|| (lastLineOfJTF.indexOf("You got to fill this field!") != -1)) {
							toWriteString = new StringBuffer();
							jtf.setText(jtf.getText() + "You got to fill this field!\n");
							jtf.setCaretPosition(jtf.getText().length());
							return;
						}
					}
					...
					try {
						...
						System.err.println("Couldn't flush the settings to OutputStream.");
					}
				}
