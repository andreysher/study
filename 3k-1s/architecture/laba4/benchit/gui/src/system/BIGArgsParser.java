/******************************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGArgsParser.java Author:
 * SWTP Nagel 1 Last change by: $Author: rschoene $ $Revision: 1.3 $ $Date: 2005/12/07 10:25:24 $
 ******************************************************************************/
package system;
 ...
 * -debug=5<BR>
 * </code><BR>
 * Of course there can be more then one argument and the order doesn't play any role.
 * 
...
public class BIGArgsParser {
	...
	public void parse(String[] args) throws BIGParserException {
		...
		while (pos < args.length) {
			...
			if (debug) {
				System.out.println("BIGArgsParser: line " + pos + " org = " + arg);
			}
			...
			if (arg.startsWith("-debug")) {
				...
				if (arg.equals("")) {
					...
					if ((level.equals("on")) || (level.equals("yes")) || (level.equals("true"))) {
						level = "1";
					}
					if ((level.equals("off")) || (level.equals("no")) || (level.equals("false"))) {
						level = "0";
					}
					if (debug) {
						System.out.println("BIGArgsParser: what: " + what + " level: " + level);
					}
					try {
						db.setDebug(what, Integer.valueOf(level).intValue());
					} catch (Exception e) {
						errorText.add("pos " + pos + ": [" + args[pos] + "] is not a valid argument");
					}
				}
				arg = "";
			}
			...
			if (arg.startsWith("-config")) {
				...
				if (arg.indexOf("=") >= 0) {
					...
					db.setConfigFile(fileName);
					if (debug) {
						System.out.println("BIGArgsParser: configFilename: [" + fileName + "]");
					}
				} else {
					if (arg.replaceAll(" ", "").equals("")) {
						errorText.add("pos " + pos + ": forgotten config filename?");
					} else {
						errorText.add("pos " + pos + ": [" + args[pos] + "] is not a valid argument");
					}
				}
			...
			/*
			 * if (arg.startsWith("-admin")) { db.setLoadAdminTool(true); arg = ""; }
			 */

			// allow admintool to create new entries or not
			/*
			 * if (arg.startsWith("-enableAdminNew")) { db.setShowAdminNew(true); arg = ""; }
			 */
