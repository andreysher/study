 ...
 * </p>
 * <p>
 * Beschreibung: BIGUpdate is used to replace complete files. Nevertheless it is necessary to edit lines in text files.
 * That's the job of BIGTextfileUpdate. BIGTextfileUpdate reads a file on the BenchIT server like jobs.txt where
 * affected file, affected string,replace string and so on are specified. After that the specified tasks are executed.
 * </p>
 * <p>
...
public class BIGTextfileUpdate {
	/**
	 * variable to synchronize searching for updates, the variable is used by the two threads t1 and t2
	 */
	private static Integer synchVar = new Integer(0);

	/**
	 * This method tests if updates for some text files are available. To do so a connection to the specified update
	 * server is established and the file 'jobs.txt' is opened to look for entries with a newer version id than the own
	 * one. If such etries are found then this method retutns true else false.
	 * 
	...
	public static boolean isTextfileUpdateAvailable(String serverAsString) {
		...
		try {
			...
			System.err.println("This URL is malformed. Try s.th. like " + BIGUpdate.DEFAULTUPDATEURL);
			return false;
		}
		try {
			txt = new URL(server.toExternalForm() + "jobs.txt");
		} catch (MalformedURLException ex) {
			System.err.println("It was tried to create a URL from " + server.toExternalForm()
					+ "jobs.txt.");
			System.err.println("This URL is malformed. Try s.th. like " + BIGUpdate.DEFAULTUPDATEURL);
			return false;
		}
		...
		try {
			...
			try {
				...
				System.err
						.println("File " + server.getPath() + "jobs.txt not found. Update not available.");
				return false;
			}
	...
	public static void update(String server, boolean consoleMsg) {
		...
		try {
			...
			if (debug) {
				System.out.println("let's start with work");
			}
			...
			if (debug) {
				System.out.println("extract tasks");
			}
			String tasks = sb.toString();
			if (debug) {
				System.out.println("A list of all tasks:\n" + tasks);
			}
			...
			if (debug) {
				System.out.println("newVersion: " + newJobVersion);
			}
			...
			if (debug) {
				System.out.println("index of own version: " + tasks.indexOf("\n" + ownVersion + "\n"));
			}
			if (tasks.indexOf("\n" + ownVersion + "\n") >= 0) {
			...
			if (debug) {
				System.out.println("now jobList.fillList() is started");
			}
			...
			/*
			 * File updateFolder=new File( BIGInterface.getInstance().getGUIPath() + File.separator + "update");
			 * updateFolder.mkdirs(); while (!updateFolder.exists());
			 */

			Iterator<BIGJobListElement> iter = jobList.iterator();

			while (iter.hasNext()) {
				BIGJobListElement ele = iter.next();
				if (debug) {
					System.err.println("Perform update for: " + ele.versionID);
				}
				performUpdate(ele);
			}

			// TODO: write new version only if update was executed successfully
			BIGFileHelper.saveToFile(newJobVersion, new File(BIGInterface.getInstance().getConfigPath()
	 ...
	 * 
	 * @param element - determines all task that has to be executed (which file, affected string, replacement string)
	 */
	private static void performUpdate(BIGJobListElement element) {
		...
		if (element.getAffectedFile().equals("BGUI.cfg")) {
			...
			if (debug) {
				System.out.println("performUpdate: BGUI.cfg");
			}
		} else {
			path = BIGInterface.getInstance().getConfigPath() + File.separator
					+ element.getAffectedFile();
			cfgParser = new BIGConfigFileParser(path);
			if (debug) {
				System.out.println("performUpdate: " + path.toString());
			}
		}
		if (debug) {
			System.out.println("performUpdate: " + element.getAffectedString() + " is set to "
					+ element.getReplaceWith());
		}
