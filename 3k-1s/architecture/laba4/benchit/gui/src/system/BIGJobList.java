...
/**
 * A textfile update can consists of different tasks. This class implements a data structure to hold all tasks of an
 * update.
 * 
 * @author Ronny Tschueter
 */
public class BIGJobList extends ArrayList<BIGJobListElement> {
	/**
	 * 
	 */
	...
	/**
	 * This method gets a tokenizer that contains the elemantary parts (affectedFile, affectedString, replaceWith) of the
	 * jobs.txt-file on the update server. It creates a BIGJobListElement for each task and set appropriate values.
	 * 
	...
	public boolean fillList(StringTokenizer tokenizer) {
		...
		if (tokenizer.hasMoreTokens()) {
			...
			if (debug) {
				System.out.println("First if statement: " + str);
			}
			try {
		...
		while (tokenizer.hasMoreTokens()) {
			String str = tokenizer.nextToken();
			if (debug) {
				System.out.println("while statement: " + str);
			}
			...
			if (str.startsWith(BIGTextfileUpdate.AFFECTEDFILE)) {
				...
				if (debug) {
					System.out.println("AFFECTED FILE IS SETTED");
				}
			} else if (str.startsWith(BIGTextfileUpdate.AFFECTEDSTRING)) {
				if (debug) {
					System.out.println("AFFECTEDSTRING");
				}
				str = str.substring(str.indexOf("=") + 1, str.length()).trim();
				...
				try {
					...
					if (debug) {
						System.out.println("The version.txt file has an invalid format.");
					}
					return false;
				}
			}
		...
		/*
		 * System.err.println("reverse"); Iterator iter = this.iterator(); while(iter.hasNext()) { System.err.println( (
		 * (BIGJobListElement) iter.next() ).versionID ); }
		 */
	...
	/*
	 * private void printStringTokenizer(StringTokenizer st) { if (debug) System.out.println("BEGIN StringTokenizer");
	 * while (st.hasMoreTokens()) { if (debug) System.out.println( st.nextToken() ); } if (debug)
	 * System.out.println("END StringTokenizer"); }
	 */
