...
public class BIGUpdate {
	...
	// the default URL for receiving updates
	public static final String DEFAULTUPDATEURL = "http://www.benchit.org/downloads/update/";
	 ...
	 * isUpdateAvailable
	 * 
	 * @param server String: e.g. http:/www.benchit.org/downloads/update in the folder should be 2 files. BenchIT.jar and
	 *          version.txt BenchIT.jar is the executable jar for the GUI and versions.txt should contain s.th like 1.2 or
	 *          0.99
	...
	public static boolean isUpdateAvailable(String server) {
		...
		/*
		 * try { urlConn = txt.openConnection(); urlConn.setReadTimeout(5000); urlConn.getContentLength(); } catch
		 * (IOException ioe) { System.err.println("File version.txt not found"); return false; }
		 */

		/*
		 * try { txt.openConnection().getContentLength() ; } catch ( IOException ex2 ) {
		 * System.err.println("File version.txt not found"); return false; }
		 */
		...
		try {
			...
			try {
				...
				System.err.println("File " + server + "version.txt not found. Update not available.");
				return false;
			}
		...
		if (BIGInterface.getVersion() < version)
			return true;
		else
			return false;
	}
	...
	public static void update(String server, boolean consoleMsg) {
		BIGInterface.getInstance();
		double ownVersion = BIGInterface.getVersion();
		...
		try {
			...
			File f;
			File origF;
			File updateFolder = new File(BIGInterface.getInstance().getConfigPath() + File.separator
					+ "update");
			updateFolder.mkdirs();
			...
			while (it.hasNext()) {
				...
				do {
					...
					f = new File(BIGInterface.getInstance().getGUIPath() + File.separator + "update"
							+ File.separator + entry);
					origF = new File(BIGInterface.getInstance().getGUIPath() + File.separator + entry);
					...
					if (BIGInterface.getSystem() == BIGInterface.WINDOWS_SYSTEM) {
						...
						String oldName = BIGInterface.getInstance().getGUIPath() + File.separator + "update"
								+ File.separator + entry;
						...
						oldName = BIGInterface.getInstance().getGUIPath() + File.separator + entry;
						for (int i = 0; i < oldName.length(); i++) {
							if (oldName.charAt(i) == '/') {
								newName.append('\\');
							} else {
						...
						/*
						 * f = new File( BIGInterface.getInstance().getGUIPath() + File.separator + "update" + File.separator +
						 * entry.replaceAll( "/" , "\\" ) ) ; origF=new File( BIGInterface.getInstance().getGUIPath() +
						 * File.separator + entry.replaceAll( "/" , "\\" ) ) ;
						 */
				...
				BIGFileHelper.saveToFile(newFileVersion, new File(BIGInterface.getInstance()
						.getConfigPath() + File.separator + "version.txt"));
			}
			BIGFileHelper.remove(updateFolder);
