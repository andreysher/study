/******************************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGDefFileParser.java Author:
 * SWTP Nagel 1 Last change by: $Author: tschuet $ $Revision: 1.4 $ $Date: 2007/05/08 09:41:53 $
 ******************************************************************************/
package system;
...
public class BIGDefFileParser {
	...
	private Object convertToType(String org, BIGEntry entry) throws NumberFormatException {
		if (org == null)
			return null;
		if (entry == null)
			return null;
		...
		if (commentPos > 0) {
			org = org.substring(0, commentPos);
		...
		switch (entry.getType()) {
			case None :
				entry.setType(BIGType.String);
				result = org;
				break;
			case String :
				if (org.startsWith("\"") && org.endsWith("\"")) {
					result = (org.substring(1, org.length() - 1)).replaceAll("\"\"", "\"");
				} else {
					result = org;
				}
				break;
			...
			case Float :
				...
				break;
			case Boolean :
				if (org.equals("1") || org.toUpperCase().equals("TRUE")) {
					result = Boolean.TRUE;
				} else {
	...
	/**
	 * Parses a given text into the database, BUT SHOULD NOT BE USED. This is for testing only, because it adds also
	 * parameters which are not in the config file. For adding all use:
	 * <code>parse(text,dbFilename,BIGInterface.getInstance().getAllEntrynames());</code>
	 * 
	 * @param text the text given as an <code>BIGStrings</code> of <code>String</code>s
	 * @param dbFilename the name of the file as it should be stored in the database
	 **/
	public void parse(BIGStrings text, String dbFilename) throws BIGParserException {
		if (debug) {
			System.out.println("BIGDefFileParser: starting parsing on " + dbFilename);
		}
		...
		while (textIterator.hasNext()) {
			...
			} else if ((!line.startsWith("#")) && (!line.equals("")) && (line.indexOf("=") >= 0)) {
				// name extraction
			...
			if (name != null) {
				...
				if (entry == null) {
					...
					if (debug) {
						System.out.println("BIGDefFileParser: ##ADDED## " + entry.getName() + ":"
								+ entry.getValue());
					}
				} else {
					try {
					...
					if (debug) {
						System.out.println("BIGDefFileParser: ##CHANGED## " + entry.getName() + ":"
								+ entry.getValue());
					}
				}
				if (debug) {
					System.out.println("BIGDefFileParser: " + line + " -> [" + name + "]:=[" + value + "]");
				}
			} else {
				if (debug) {
					System.out.println("BIGDefFileParser: " + line + " -> FILTERED");
				}
			}
		}
		if (errorText.size() > 0)
	 ...
	 * @param text the text given as an <code>BIGStrings</code> of <code>String</code>s
	 * @param dbFilename the name of the file as it should be stored in the database
	 * @param entrynames a list of all names of entries that should be recognized, all others are ignored
	 **/
	public void parse(BIGStrings text, String dbFilename, BIGStrings entrynames)
		...
		Iterator<String> defaultIt = entrynames.iterator();
		while (defaultIt.hasNext()) {
			BIGEntry entry = db.getEntry(dbFilename, (defaultIt.next()));
			if (entry == null)
				continue;
			if (entry.getPrototypeValue() != null) {
		...
		while (textIterator.hasNext()) {
			...
			if (entrynames.contains(name)) {
				...
				if (entry == null) {
					...
					try {
						entry.setValue(convertToType(value, entry));
					} catch (NumberFormatException ex) {} catch (BIGAccessViolationException ex) {}
					/*
					 * } catch (Exception e) { //entry.setValue(null); errorText.add("type convertion failed on " + name); }
					 */
					db.addEntry(dbFilename, entry);
					if (debug) {
						System.out.println("BIGDefFileParser: ##ADDED## " + entry.getName() + ":"
					...
					try {
						entry.setValue(convertToType(value, entry));
					} catch (NumberFormatException ex1) {} catch (BIGAccessViolationException ex1) {}
	...
	/**
	 * Reads a Def-File in the database, BUT SHOULD NOT BE USED. This is for testing only, because it adds also parameters
	 * which are not in the config file. For adding all use:
	 ...
	 * @param dbFilename the name of the file as it should be stored in the database
	 **/
	public void read(String realFilename, String dbFilename) throws BIGParserException {
		BIGStrings text = new BIGStrings();
		try {
			text.readFromFile(realFilename);
		} catch (FileNotFoundException fnfe) {
			System.err.println("BIGDefFileParser: File [" + realFilename + "] not found, skipping.");
		} catch (IOException ioe) {
			System.err.println("BIGDefFileParser: Error during reading file [" + realFilename
					+ "], skipping.");
		}
	 ...
	 * @param entrynames a list of all names of entries that should be recognized, all others are ignored
	 **/
	public void read(String realFilename, String dbFilename, BIGStrings entrynames)
			throws BIGParserException {
		BIGStrings text = new BIGStrings();
		try {
			text.readFromFile(realFilename);
	 ...
	 * 
	 * @param originalText the original text to render in given as an <code>BIGStrings</code>
	 * @param dbFilename the name of the file in database what should be rendered
	 * @param entrynames a list of all names of entries that should be recognized, all others are ignored
	 **/
	public BIGStrings render(BIGStrings originalText, String dbFilename, BIGStrings entrynames)
			throws BIGParserException {
		// var declaration
		BIGStrings errorText = new BIGStrings();
		BIGInterface db = BIGInterface.getInstance();
		...
		while (lineNr < originalText.size()) {
			...
			if (line.startsWith("#start of ")) {
				...
				if (entrynames.contains(name)) {
					...
					while (lineNr < originalText.size() && !end) {
						lineNr++;
						tempLine = originalText.get(lineNr);
						if (tempLine.equals("#end of " + name)) {
							end = true;
						} else {
							value = value + tempLine + "\n";
			...
			if ((!line.startsWith("#")) && (!line.equals("")) && (line.indexOf("=") >= 0)) {
				// name extraction
				name = line.substring(0, line.indexOf("=")).trim();
			...
			if (entrynames.contains(name)) {
				...
				if (entry == null) {
					...
					errorText.add("should change a value what's not in database: " + name);
					// so do nothing
					newText.add(originalText.get(lineNr));

				} else {
					if (entry.getType() == BIGType.FreeText) {
						try {
						...
						/*
						 * System.err.println( "Save A:" + originalText.get( lineNr ).substring( 0 , originalText.get( lineNr
						 * ).indexOf( "=" ) + 1 ) + convertToString( entry ));
						 */

						newText.add(originalText.get(lineNr).substring(0,
		...
		while (i < entrynames.size()) {
			...
			if (entry == null) {
				...
				if (((entry.getValue() != null) && (entry.getValue().equals(entry.getPrototypeValue())))
						|| (entry.getType() == BIGType.None)) {
					entrynames.remove(i);
				} else {
					i++;
				}
		...
		while (doneIterator.hasNext()) {
			percentDone++;
			progress.setProgress(((50 * percentDone) / entrynames.size()) + 50);
			...
			if (!doneEntrynames.contains(addName)) {
				...
				if (!found) {
					...
					if (entry == null) {
						// should change value but there is no entry in database
						errorText.add("should add a value what's not in database: " + addName);
					} else {
						if (entry.getType() == BIGType.FreeText) {
				...
				// if found
					...
					if (!found) {
						...
						if (entry == null) {
							...
							newText.add(0, addName + "=" + convertToString(entry));
						}
					} else {
						// now we have to search the marker in the new text
						String markerName = line.replaceAll(" ", "");
						...
						while ((lineNr < newText.size()) && (!added)) {
							...
							if (line.startsWith("#start of " + addName)) {
								BIGEntry entry = db.getEntry(dbFilename, addName);
								newText.add(lineNr + 1, convertToString(entry));
								added = true;
							} else
							...
							if ((!line.startsWith("#")) && (!line.equals("")) && (line.indexOf("=") < 0)) {
								...
								if (markerName.equals(line)) {
									...
									if (entry == null) {
										...
										newText.add(lineNr + 1, addName + "=" + convertToString(entry));
									}
								} else {
									// was another marker
									lineNr++;
	 ...
	 * 
	 * @param realFilename the name of the file on the disk
	 * @param dbFilename the name of the file as it is stored in the database
	 * @param entrynames a list of all names of entries that should be recognized, all others are ignored
	 **/
	public void write(String realFilename, String dbFilename, BIGStrings entrynames)
			throws BIGParserException {
		...
		try {
			...
			try {
				orgText.readFromFile(db.getBenchItPath() + "\\LOCALDEFS\\"
						+ dbFilename.replaceAll("<hostname>", "PROTOTYPE"));
