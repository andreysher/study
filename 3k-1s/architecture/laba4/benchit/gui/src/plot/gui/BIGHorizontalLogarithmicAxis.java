...
/**
 * A logarithmic value axis, for values displayed horizontally. Display of positive values arbitrarily close to zero is
 * supported, as well as negative values (if 'allowNegativesFlag' flag set).
 * 
 * @author Eric Thomas
 */
public class BIGHorizontalLogarithmicAxis extends org.jfree.chart.axis.HorizontalNumberAxis
		implements
			Serializable {
	...
	/** If number formatter is set in plot config, this will be used- */
	 ...
	 */
	public BIGHorizontalLogarithmicAxis(String label, BIGPlotable plotable, String format) {
		...
		allowNegativesFlag = false; // save flag
		setLog(10);
	...
	public BIGHorizontalLogarithmicAxis(String label, int log, BIGPlotable plotable, String format) {
		// set the default min/max axis values for a logarithmic scale.
	...
	/**
	 * Sets the 'allowNegativesFlag' flag; true to allow negative values in data, false to be able to plot positive values
	...
	/**
	 * Returns the 'allowNegativesFlag' flag; true to allow negative values in data, false to be able to plot positive
	 ...
	 * plotArea. Note that it is possible for the coordinate to fall outside the plotArea.
	 * 
	...
	/**
	 * Converts a coordinate in Java2D space to the corresponding data value, assuming that the axis runs along one edge
	 * of the specified plotArea.
	 * 
	...
	public double translateJava2DtoValue(float java2DValue, Rectangle2D plotArea) {
		...
		if (isInverted())
			return Math.pow(log, axisMax - ((java2DValue - plotX) / (plotMaxX - plotX))
					* (axisMax - axisMin));
		else
			return Math.pow(log, axisMin + ((java2DValue - plotX) / (plotMaxX - plotX))
					* (axisMax - axisMin));
	}
	...
	/**
	 * Returns the smallest (closest to negative infinity) double value that is not less than the argument, is equal to a
	 * mathematical integer and satisfying the condition that log base 10 of the value is an integer (i.e., the value
	...
	/**
	 * Returns the largest (closest to positive infinity) double value that is not greater than the argument, is equal to
	 * a mathematical integer and satisfying the condition that log base 10 of the value is an integer (i.e., the value
	 * returned will be a power of 10: 1, 10, 100, 1000, etc.).
	 * 
	...
	@Override
	public void refreshTicks(Graphics2D g2, Rectangle2D drawArea, Rectangle2D dataArea, int location) {
		getTicks().clear();
		...
		/*
		 * for (int i = iBegCount; i <= iEndCount; i++) { //for each power of 10 value; create ten ticks for (int j = 0; j <
		 * log; ++j) {
		 */
		for (int i = iBegCount; i <= iEndCount; i++) {
			...
			for (int j = 0; j < jEndCount; j++) {
				...
				if (smallLogFlag) {
					...
					tickVal = Math.pow(log, i) + (Math.pow(log, i) * j);
					if (j == 0) {
						// first tick of group; create label text
						if (logTickLabelsFlag) {
							// if flag then
							// if 10, 100 or 1000 replace base^i with 1E display
							switch (log) {
								case 10 :
									tickLabel = "1E" + i;
									break;
					...
					tickVal = (i >= 0)
							? Math.pow(log, i) + (Math.pow(log, i) * j)
							: -(Math.pow(log, -i) - (Math.pow(log, -i - 1) * j));
					if (j == 0) { // first tick of group
						if (!zeroTickFlag) { // did not do zero tick last
							...
							if (i > iBegCount && i < iEndCount && Math.abs(tickVal - 1.0) < 0.0001) {
								// not first or last tick on graph and value is
								// 1.0
								...
								// create label for tick ("log10" label if
								// flag):
								// if 10, 100 or 1000 replace base^i with 1E display
								switch (log) {
									case 10 :
				...
				Rectangle2D tickLabelBounds = getTickLabelFont().getStringBounds(tickLabel,
				...
				if ((tickVal >= getMinimumAxisValue()) && (tickVal <= getMaximumAxisValue())) {
	...
	/**
	 * Returns an adjusted log10 value for graphing purposes. The first adjustment is that negative values are changed to
	 * positive during the calculations, and then the answer is negated at the end. The second is that, for values less
	 * than 10, an increasingly large (0 to 1) scaling factor is added such that at 0 the value is adjusted to 1,
	 * resulting in a returned result of 0.
	 * 
	 * @param val value for which log10 should be calculated.
