 ...
 * BIGVerticalLogarithmicAxis.java Author: Robert Schoene Last change by: $Author: tschuet $
 * $Revision: 1.10 $ $Date: 2007/05/08 09:48:35 $
 *********************************************************************/
...
/**
 * A logarithmic value axis, for values displayed vertically. Display of positive values arbitrarily close to zero is
 * supported, as well as negative values (if 'allowNegativesFlag' flag set). based on JFreeChart 0.9.8
 * 
...
public class BIGVerticalLogarithmicAxis extends VerticalNumberAxis implements Serializable {
	 ...
	 */
	public BIGVerticalLogarithmicAxis(String label, int newLog, BIGPlotable plotable, String format) {
	 ...
	 * @param label the axis label (null permitted).
	 */
	public BIGVerticalLogarithmicAxis(String label, BIGPlotable plotable, String format) {
	...
	public double translateJava2DtoValue(float java2DValue, Rectangle2D dataArea) {
		...
		if (isInverted())
			return Math.pow(log, axisMin + ((java2DValue - plotY) / (plotMaxY - plotY))
					* (axisMax - axisMin));
		else
			return Math.pow(log, axisMax - ((java2DValue - plotY) / (plotMaxY - plotY))
					* (axisMax - axisMin));
	}
	...
	protected double computeLogFloor(double lower) {
		...
		if (allowNegativesFlag) {
			...
			} else if (lower < log) { // parameter value is < -10
				// calculate log using positive value:
				logFloor = Math.log(-lower) / LOG_VALUE;
	...
	public void refreshTicks(Graphics2D g2, Rectangle2D plotArea, Rectangle2D dataArea, int location) {
		...
		getTicks().clear();
		setTickLabelFont(plotable.getFont("YAxisTick"));
					...
					if (j == 0) {
						...
						if (logTickLabelsFlag) {
							...
							if (i >= 0) { // if positive exponent then make
								...
								System.out.println("BIGVerticalLogarithmic numberFormat: " + tickVal);
							}
						}
	 ...
	 * VerticalNumberAxis and VerticalDateAxis. had to be reimplemented to remove file-output-bug under java 1.5
	 * 
