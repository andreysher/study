/******************************************************************************
 * B e n c h I T - Performance Measurement for Scientific Applications BIGMain.java Author: SWTP
 * Nagel 1 Last change by: $Author: tschuet $ $Revision: 1.4 $ $Date: 2007/05/08 09:35:42 $
 ******************************************************************************/
...
/**
 * This is the main class for loading the application. Also you can load the AdminTool if you add -admin to the command
 * line arguments. <br>
 * <br>
...
public class BIGMain {
	...
	/**
	 * a thread, which starts the gui, while the other one (handling the splash and init) ends
	 */
	...
	/**
	 * This method is needed, because, if there are no files, that define the System, the save doesnt really work so this
	 * checks the files and (if needed) replaces them with the PROTOTYPE files
	 * 
	...
	private void copyPrototypeIfNeeded(String benchitPath, String hostname) {
		...
		for (int i = 0; i < filenames.length; i++) {
			...
			try {
				BufferedReader in = new BufferedReader(new FileReader(benchitPath + File.separator
						+ "LOCALDEFS" + File.separator + hostname + filenameext));
				in.close();
			} catch (Exception e) { // FIRSTTIME must be called here !!!!!!
				...
				try {
					...
					in = new FileInputStream(benchitPath + File.separator + "LOCALDEFS" + File.separator
							+ "PROTOTYPE" + filenameext);
					// and write files with out
					out = new FileOutputStream(benchitPath + File.separator + "LOCALDEFS" + File.separator
							+ hostname + filenameext);
	...
	public BIGMain(final String[] args) {
		...
		startThread = new Thread() {
			...
			public void run() {
				...
				// set the hostname
				bigInterface.setHost(BIGInterface.getInstance().getOriginalHost());
				...
				fileParser = new BIGConfigFileParser(bigInterface.getConfigPath() + File.separator
						+ "BGUI.cfg");
				...
				ImageIcon teamIcon = new ImageIcon(bigInterface.getImgPath() + File.separator
						+ "splash.jpg");
				// once the team was displayed here. to salute the swt-peaople it stays ;)
				JLabel teamLabel = new JLabel(teamIcon);
				teamLabel.setBounds(0, 0, teamIcon.getIconWidth(), teamIcon.getIconHeight());
				...
				progressLabel.setFont(new Font(progressLabel.getFont().getName(), progressLabel.getFont()
						.getStyle(), 10));
				...
				addComponent(progressLabel, 0, 1, 1, 1, progressPanel, GridBagConstraints.HORIZONTAL,
						GridBagConstraints.CENTER, 100, 0);
				createStrut(0, 2, 1, 1, spaceLeftRight, 10, progressPanel);
				createStrut(1, 0, 1, 1, spaceLeftRight, 10, progressPanel);
				addComponent(startProgress, 1, 1, 1, 1, progressPanel, GridBagConstraints.HORIZONTAL,
						GridBagConstraints.CENTER, 100, 0);
				createStrut(1, 2, 1, 1, spaceLeftRight, 10, progressPanel);

				startWindow.getContentPane().add(progressPanel, BorderLayout.SOUTH);
				startWindow.getLayeredPane().add(teamLabel, new Integer(Integer.MIN_VALUE));
				...
				startWindow.setBounds((w - teamIcon.getIconWidth()) / 2,
						(h - teamIcon.getIconHeight()) / 2, teamIcon.getIconWidth(), teamIcon.getIconHeight());
				// finally, the user sees it
				startWindow.setVisible(true);
				// (new
				// MouseEvent(starWindow,12345,System.currentTimeMillis(),0,1,1,1,false,MouseEvent.BUTTON1))
				...
				try {
					...
					console
							.postMessage("Cannot parse these command line arguments: " + bpe, BIGConsole.ERROR);
				}
				// if we are new on the system copy PROTOTYPE-LOCALDEFS to new LOCALDEFS
				// (the gui needs LOCALDEFS to work on)
				copyPrototypeIfNeeded(bigInterface.getBenchItPath(), bigInterface.getHost());
				...
				try {
					...
					console.postMessage("error during loading database in BIGMain: " + bpe, BIGConsole.ERROR);
					console = null;
					System.gc();
					System.err.println("error during loading database in BIGMain: " + bpe);
					System.exit(1);
				} catch (BIGDoubleIdentifierException bdie) {
					console.postMessage("error by adding a file in BIGMain: " + bdie, BIGConsole.ERROR);
					console = null;
					System.gc();
				...
				int rightSplit = -1, mainSplit = -1;
				DetailLevel detailLevel = DetailLevel.Low;
				try {
					...
					detailLevel = fileParser.DetailevelCheckOut("detailLevel");
				} catch (Exception ignored) {}

				// the negoation of the boolean tells BIGGUI, wether it
				// has still to load sth or not.
				gui = new BIGGUI(startProgress, progressLabel);
				gui.setValues(x, y, xSize, ySize, rightSplit, mainSplit, detailLevel);
